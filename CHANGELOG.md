...
- **Switched the security-scan runner from Semgrep to Opengrep (closes #111)** — [Opengrep](https://github.com/opengrep/opengrep) is an open, login-free fork of Semgrep with the same rule format and a semgrep-compatible CLI. The decisive difference: it ships as a **single standalone binary per platform** on GitHub releases with **no account, token, or telemetry**, so pi-lens can now **auto-install it on demand** via the existing `github` single-binary strategy — where Semgrep was deliberately *never* auto-installed (it needed pipx/brew + optional login). The runner (`opengrep`, replacing `semgrep`) auto-installs when elected, then runs `opengrep scan --json --timeout 5 --config <cfg> <file>`; note **no `--metrics=off`** (Opengrep has no metrics flag — it never phones home — so passing it would error). User surface is **seamless and minimal** (no semgrep aliases): a single `--lens-opengrep` flag that is **self-sufficient** — with no repo rule file it defaults to `--config auto` (Opengrep's login-free Community ruleset, ~1k rules); `--lens-opengrep-config <auto|p/pack|path>` overrides and implies it. A repo `.opengrep.yml`/`.opengrep.yaml` (or legacy `.semgrep.yml`/`.semgrep.yaml`, which Opengrep consumes natively) **auto-enables** dispatch — the rule file is the opt-in signal. There is **no `/lens-opengrep` command and no persisted `.pi-lens/opengrep.json`**: that Semgrep-era ceremony (status/init/enable/disable/clear) managed install/token/config frictions that Opengrep's auto-install + login-free rules erase, so it was dropped as vestigial. Stays off by default otherwise (a network-fetching, hot-path security scanner shouldn't run without consent). Wired via `withOpengrepGroup` (the dynamic opt-in dispatch group, sibling to `withSpotbugsGroup`) and registered in `TOOL_EXECUTION_POLICY` (config-first, auto-install) + `GITHUB_TOOLS`. **Validated end-to-end on the dev box**: opengrep 1.22.0 auto-installs and runs. Guards: `opengrep-config.test.ts` (resolution: local-file/flag-defaults-to-auto/config-implies-enable/disabled + `.semgrep.yml` compat + arg normalization), `opengrep-parser.test.ts` (severity/semantic/defect-class mapping + high-signal-security promotion + path fallback + MAX_DIAGNOSTICS cap), and the github install-contract/full-matrix guards. _(FindSecBugs/PMD remain noted JVM follow-ups; unrelated.)_

### Added

- **SpotBugs bytecode bug-pattern analyzer for Java + Kotlin (closes #133)** — Java's pipeline was `javac`-only (compile errors, no static analysis); SpotBugs (Apache-2.0) adds 400+ bytecode-level bug patterns (null derefs, resource leaks, thread-safety, performance, bad-practice), and since it analyzes JVM **bytecode**, Kotlin projects get it for free. **Opt-in** behind the `lens-spotbugs` flag (it's heavyweight — JVM cold start + whole-tree analysis), wired via a `withSpotbugsGroup` dispatch group (sibling to `withOpengrepGroup`) that only activates when the flag is set **and** a Java build descriptor (`pom.xml`/`build.gradle{.kts}`/`settings.gradle{.kts}`) **and** a compiled-classes dir (`target/classes`, `build/classes`, `out/production`, `bin/main`) are present. The runner operates on the compiled tree (not the edited source) and **mtime-caches** — it only re-invokes SpotBugs after a rebuild changes the `.class` files, returning cached findings otherwise. `<BugInstance>` XML (`-xml:withMessages`) is parsed by a bounded, zero-dep, ReDoS-safe reader: priority→severity (1=error/2=warning/3=info), category→defect-class (`CORRECTNESS`/`MT_CORRECTNESS`→correctness, `SECURITY`→safety, `PERFORMANCE`/`BAD_PRACTICE`/`STYLE`/`I18N`/`EXPERIMENTAL`→style), `type`→rule, the primary `<SourceLine>`→location, and the `<LongMessage>` first sentence→`fixSuggestion`; bugs stay advisory (`semantic: warning`). Auto-installs via the archive strategy. **Validated end-to-end on the dev box**: a `NP_ALWAYS_NULL` null-deref in a compiled Maven fixture surfaced as `error`/`correctness` at the right line, through the real dispatch path (coexisting with javac). Guards: `spotbugs-parser.test.ts` (severity×category mapping + primary-line selection + drop-no-source), `spotbugs-runner.test.ts` (scan/cache/rebuild-reinvoke/skip-when-unbuilt), and detection-helper + dispatch-coverage/smoke-coverage exemptions. _(FindSecBugs security plugin + PMD are noted follow-ups.)_
