...
- **Bounded `workspace/symbol` and `textDocument/codeAction` LSP requests (#365)** — like the pull-diagnostics fix (#364), these two requests were sent via `safeSendRequest` with no `withTimeout` ceiling, so a language server that accepts the request but never replies (alive but hung) would make the `await` never resolve — hanging symbol search (`pilens_symbol_search`) and code-action lookups. Both now route through the shared `navRequest` helper (its `withTimeout` ceiling + single-file stale-drop), so a hung server times out and returns `[]` instead of hanging; real errors still propagate. (`workspace/executeCommand` — mutating and legitimately long-running — is tracked separately for a considered ceiling.)
