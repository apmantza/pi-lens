...
- **Read-guard mixed-validity multi-edits preserve the host edit tool's atomic semantics by default** (closes #637, refs #144) — when one or more `oldText` blocks fail preflight, pi-lens now blocks the whole batch without writing the blocks that did match. The former partial-apply behavior is retained only as an explicit opt-in via `--lens-partial-edit-apply` or global config `edit.partialApply: true`; atomic failures say that no changes were written, while an opted-in partial-apply pipeline failure conservatively tells the agent to re-read because disk state may have changed. Config parsing, flag wiring, preflight messaging, and both default/opt-in disk behavior have regression coverage.
