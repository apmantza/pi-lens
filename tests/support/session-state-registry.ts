...
export const SESSION_STATE_REGISTRY: SessionStateEntry[] = [
	...
	},
	{
		...
		reason:
			"#2858: the dead-weight line is one row per pi conversation or MCP connection naming situational tools that host never used. Reload, resume, and fork preserve pi observations; a fresh /new replacement emits once and resets; a process restart observes restored active tools as activations, while called is unrecoverable. MCP owns connectionEnded. The reset's sessionStarted exclusion is stated in the state field above.",
		probe: {
			arm: () => {
