...
function harness(options: { documentSymbol?: boolean } = {}): Harness {
	...
	vi.mocked(state.connection.sendRequest).mockImplementation((async (
		...
		wire.push(String(method));
		if (method === "textDocument/documentSymbol") return fence.promise;
		return undefined;
	}) as never);
	...
	return { state, publish, wire, change, fence };
}
...
describe("#3484 — diagnostics fence for version-less servers", () => {
	...
	it.each([
		...
		{
			branch: "a reopenOnResync close + reopen (the close is the first send)",
			open: true,
			serverId: "opengrep",
			send: (s: LSPClientState) =>
				handleNotifyOpen(s, FILE, "content 1", "typescript"),
			first: "textDocument/didClose",
		},
	...
	it("sends no fence to a versioned server, whose own guards drop the stale publish", async () => {
		const h = harness();
		await primed(h, true);
	...
	it("sends no fence before the server has published at all", async () => {
		const h = harness();
		h.change.resolve();

		await handleNotifyOpen(h.state, FILE, "content 1", "typescript");

		expect(h.wire).toEqual(["textDocument/didChange"]);
	});

	it("keeps today's behaviour, recorded once, when the server advertises no documentSymbol", async () => {
