...
import { realHarnessFixtureRoot, validateScript } from "../support/real-pi-harness";
...
describe("real harness fixture shape", () => {
	it("requires every scenario to provide a project directory and valid script fields", () => {
		for (const scenario of readdirSync(realHarnessFixtureRoot)) {
			...
			const script = JSON.parse(readFileSync(path.join(dir, "script.json"), "utf8")) as unknown;
			expect(() => validateScript(script, `${scenario}/script.json`)).not.toThrow();
		}
	});
	it("names the malformed field", () => {
		expect(() => validateScript([[{ type: "text" }]], "broken/script.json")).toThrow(/text must be a string/);
	});
});
