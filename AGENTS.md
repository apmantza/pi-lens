...
`setupTestEnvironment` retains ownership of its tracked roots after per-test cleanup so deferred work can be swept if it recreates a root. Use `cleanupTestEnvironments` when a test family can recreate a root after its per-test `finally`; keep independently-created roots on their own `afterEach` cleanup.
