...
- **Bus events → EXTENSIONS.** `pilens:files:touched` on `pi.events` (#482, `clients/bus-publish.ts`) and `pilens:diagnostics` (#502, `clients/diagnostics-publish.ts`). Broadcast-only; other extensions in the session observe pi-lens's autonomous writes and findings without reverse-engineering it.
