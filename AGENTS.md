...
`setupTestEnvironment` tracks its roots for a suite-level cleanup pass. Use `cleanupTestEnvironments` when a test family can recreate a root after its per-test `finally`; keep independently-created roots on their own `afterEach` cleanup.
