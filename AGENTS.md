...
- `~/.pi-lens/latency.log` `cache_context` records are the privacy-preserving request-side context audit: session/turn, injection sources/placement, bounded counts/sizes, and hashes only. `cache_prefix_break` remains a local first-message stability signal, not proof of a provider cache miss; `cache_usage` is provider-reported and correlates only by session/turn because the host exposes no request id.
