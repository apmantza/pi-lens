...
- **A shared logger mock** → use Vitest's `importOriginal`/`importActual` pass-through with only the test's targeted overrides. A whole-module factory that names selected logger exports silently drops future exports; `tests/config/vi-mock-export-sweep.test.ts` checks the real export surface, with source scanning driven by code rather than comments or strings (#2272, #2281).
