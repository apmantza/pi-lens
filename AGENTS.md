...
- **Warm call-graph cache source identity (#1070).** `runtime-session` validates persisted call-graph mtimes against `review-graph/builder.ts`'s `getGraphSourceFiles` seam, not a raw source walk: this preserves the graph's test-file, size-limit, entry-budget, and compiled-twin policy. Warm call facts remain TypeScript/TSX-only; unsupported or partial extraction must remain explicit in coverage rather than becoming a clean empty graph. Legacy ambiguous name-only evidence may fan out to weighted edges, but coverage counts raw evidence once and persisted validation weights those edges back to raw records.
