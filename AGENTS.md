...
**Adding a shape.** Append it at the TAIL with the next free number, never in the middle. These are cited by number across briefs, contracts and issues, so renumbering an existing shape invalidates every reference to it. An unnumbered paragraph between two numbered shapes, an out-of-order number, and a gap in the sequence all end the ordered list and red markdownlint's MD029 — three separate PRs hit one of those three in a single day (#2988, #2945, #2984). If two open PRs both claim the same next number, the one that merges second renumbers.
