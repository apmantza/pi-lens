...
- **bash WRITES** (`extractWrittenPathsFromCommand`): `>`/`>>`/`N>`, `tee`, `sed -i`, `cp`/`mv` dest, `touch`. The agent authored the file only after complete post-command evidence confirms a change, so recognized paths use the evidence fence and `recordWritten` at tool_result; creation is detected when the post snapshot contains a path absent from the pre snapshot.
