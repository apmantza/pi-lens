...
- **`package-lock.json` IS committed and must stay in sync** with `package.json`. `npm run check:lockfile` (CI and the Husky pre-commit hook) fails on identity, license, bin, engine, dependency-spec, and peer-optional mirrors; after any dep or version change run `npm install` and commit the lock. The changelog release preflight reuses that same validator before it rewrites `CHANGELOG.md` or deletes fragments. The clean-clone production-install job also fails when `npm install --omit=dev` changes any tracked source file. CI/release use `npm install` (not `npm ci`) so a desync self-heals instead of wiping `node_modules`.
