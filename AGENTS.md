...
Diagnostics have one model-facing surface, `lens_diagnostics`; `source` selects `session`, `lsp`, or `analyzers`, and `scope` selects `delta`, `paths`, or `workspace`. The retired MCP name is intercepted through the canonical enabled-tool gate for one release, maps to `source=lsp`, and records `lsp-diagnostics-compatibility` once per session. The retired name stays out of `TOOL_REGISTRY`, pi/MCP rosters, skills, and user-facing docs. (#2800 item 2)
