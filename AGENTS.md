...
- **A whole-module `vi.mock` factory** → `tests/config/vi-mock-export-sweep.test.ts` and `tests/support/vi-mock-export-gate.ts` (#2281): resolve relative `.js` specifiers to `.ts`, compare the production value-export surface with exports used by the test's direct production importers, and prefer an `importOriginal`/`importActual` spread so additions pass through. The baseline is two-sided and every finding carries file, line, specifier, and missing names; #2272 and #2782 are the recurrences.
