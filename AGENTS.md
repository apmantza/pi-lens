...
40. **A tool root resolved at a different seam than its child spawn.** Runners, formatters, and LSP servers must call `resolveToolCwd(kind, tool, file, ctx)` so marker discovery, the dispatch-root fallback, the real-git fallback, and the `$HOME` ceiling cannot drift between consumers. The seam preserves tool-owned per-file configuration discovery, logs `cwd <kind> <tool> cwd=<path> reason=<...>` once per resolution key per session, and records one bounded `tool-cwd-resolution` degradation per fallback tool and session. *Screen:* sweep every runner and formatter child spawn, php-cs-fixer config carriage, and LSP root detector for a direct `ctx.cwd`, `path.dirname`, or private walk; mutation of one consumer, the log throttle, or the fallback ledger must make its corresponding test red. The existing AST sweep remains the population guard, and the seam state-space table covers nested markers, root-only markers, foreign files, symlinks, per-file versus per-cwd discovery, and LSP servers with or without `rootMarkers` (#2777, extending #2691).
	*Also:* runner-owned markers must remain in the canonical language-profile vocabulary consumed by `markersFor`, even when they are not language-root markers. A membership diff against the deleted runner table and real nested-filesystem rows for each affected tool family are required before deleting a sibling marker table (#2971, refs #2965).
...
**Finding-delivery gate — single doc home (#1634).** Every store above (secrets, govulncheck, the cached-blocker gate) plus `lens_diagnostics` mode=full/all, the widget footer, agent nudges, and the persisted project-diagnostics snapshot are enumerated in `clients/finding-delivery-gate.ts`'s `DELIVERY_SURFACES` registry — the single place the freshness/dispositions/demotion-rendering contract is spelled out; every other file that gates or labels a finding points there instead of re-explaining it. Each surface is either `gated` (routes through `gateFindingsByPathFreshness`/`applyDispositions`/the blocker-freshness family above) or `labeled` (a package-pinned finding like a trivy CRITICAL CVE has no cited path to gate, so `formatCacheAgeLabel` states the session_start cache's age instead) — `assertNoDeliveryBypass()` rejects any registration that is neither, and `tests/clients/finding-delivery-gate.test.ts` ground-truths every entry's named gate/label call against its declared source file. A disposition mark updates the widget through `applyDispositions` and `commitDiagnostics`, the same filter/count seam used by dispatch; the host consumes the published event through the same reconciler, and suppressed entries remain annotated in the shared record (#1616).
...
- **`lens_diagnostics` explicit analysis root (#2053).** `mode=full` may receive `analysisRoot` for heavyweight analyzers when the supplied path exists, is a directory, and its canonical realpath is strictly below the canonical home ceiling. `path.resolve` and `realpath` resolve `..` and symlinks before the `isAtOrAboveHomeDir` check; the root is validated once per call and never cached across calls. An invalid or at/above-home root refuses every heavyweight analyzer with an honest cold notice. The supplied root is the only walk root; `paths` still filters reported diagnostics and does not narrow analyzer walks.
