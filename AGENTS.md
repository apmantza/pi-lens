...
40. **A tool spawn without the dispatch cwd.** A config-sensitive tool (yamllint, typos, sqlfluff, ruff, PSScriptAnalyzer) discovers its project configuration by walking UP FROM THE CHILD PROCESS CWD, not from the target file, so a spawn with an absolute `ctx.filePath` and no `cwd` option lints the right file against the extension host's config — and the runner's own `hasXConfig(ctx.cwd)` gate says the project config was found. The version probe passes `cwd`; the lint spawn ten lines below does not; nothing in the result says which config was read (#2691, external, on 4.1.4). #1731 fixed this by symbol in 2026-08 (sqlfluff, taplo, biome, stylua, vulture) and left the shape unguarded, so yamllint, ruff, typos, oxlint, shellcheck and the psscriptanalyzer helper carried it for three more weeks (#2693 found six sites where the orchestrator's count-based grep had found four: a `cwd` passed as a helper ARGUMENT counted as an option) — sweep by shape, not by symbol, and ratchet the shape. *Screen:* every `safeSpawnAsync`/`safeSpawnSync` call in a runner, formatter or installer passes `cwd: ctx.cwd || process.cwd()` (the same value the availability probe and the config gate used), including spawns routed through a local helper — a helper that takes `cmd, args, timeout` and no `cwd` is the shape one layer up. Prefer `ctx.cwd` to `projectRoot`: the walk-up must start at the dispatch directory so a nested config overrides the repo-level one. *Detect:* the static sweep over runner spawn sites landed with the #2691 fix fails naming `file:line` for any spawn whose OPTIONS LITERAL lacks `cwd` — the literal, not the whole call: #2693 r1 tested the call text, so `ctx.cwd` in the command argument cleared typos and the sweep's own red block missed one of the six defects it shipped with, and a `path.resolve(cwd, file)` rewrite evaded it with the defect intact — and it follows same-file spawn wrappers, since a wrapper parameter named `cwd` is not a passed cwd (#2693 r1 F2); and it reads the VALUE, not only the key — `{ cwd: process.cwd() }`, `undefined`, `null` and `""` are the defect written out (#2693 r3 F1). The shipped detector is an AST scan on `@ast-grep/napi` (`tests/support/spawn-cwd-scan.ts`) after two text-scanner rounds each fell to a new spelling — a structural question gets a structural scan, never a regex over blanked source; for a new tool, prove config sensitivity with the issue's two-cwd probe (same absolute file, two `cd`s, different findings) before trusting either answer.
