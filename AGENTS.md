...
- **Toolchain-gated LSP auto-install (#241)** — `ensureTool` only covers servers in the installer registry (npm/pip/gem + single-binary github/maven/archive). gopls, jdtls, sourcekit-lsp, csharp-ls, haskell-language-server, ocamllsp, nixd, kotlin-language-server, clangd, lua-language-server, elixir-ls are NOT registered (need a language runtime / non-binary install). Proposal: a `runtimeInstall` step gated on toolchain presence + discovery of canonical bin dirs (`GOPATH/bin`, `~/.dotnet/tools`, …) even when not on PATH. (clojure-lsp + gleam ARE registered now — single native binaries.)
