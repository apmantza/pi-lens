...
- **Whole-module `vi.mock` factories** → `tests/config/vi-mock-export-sweep.test.ts` and `tests/support/vi-mock-export-gate.ts` (#2281): resolve relative `.js` specifiers to `.ts`, compare the production value-export surface with exports used by the test's direct production importers, and prefer an `importOriginal`/`importActual` spread so additions pass through. Skip `.mjs`, `.d.mts`, and `node:` specifiers. Keep the two-sided baseline, with every admission naming its file, line, specifier, and missing exports; #2272 and #2782 are the recurrences.
