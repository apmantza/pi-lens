...
clients/
  ...
  lsp/                    37 LSP server IDs (incl. opengrep + ast-grep, cross-cutting AUXILIARY diagnostic LSPs — role:"auxiliary", #111/#239), config, lifecycle. clojure-lsp + gleam now auto-install via github (native binary / flat tarball)
  dispatch/               Pipeline dispatcher + 47 registered runners (incl. spotbugs — flag-gated via withSpotbugsGroup, #133). Auxiliary LSPs (opengrep, …) are NOT runners — they attach via the lsp runner's with-auxiliary path; see clients/dispatch/auxiliary-lsp.ts
...
- **Per-file / per-event work must be O(1) amortized** — memoize expensive derivations keyed by an invalidation signal (`.gitignore` mtime, `fileSeq`, content hash); never recompute-from-scratch on repeat (e.g. `ignoreMatcher.isIgnored` was recomputed per file per scan — now memoized). The `getProjectIgnoreMatcher` cache also keys on the actual inherited `.pi-lens.json`/`pi-lens.json` path + mtime (not just a file directly under the git root), so editing project `ignore` patterns drops the cached matcher without a session restart.
