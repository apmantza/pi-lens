...
#   --autofix drives runAutofix (the pipeline phase that applies fixable linters in --fix mode — distinct from lint-only dispatch AND from formatters; it MUTATES files): asserts the policy-selected tool applied a fix (fixedCount>0). Covers 8/9 (ruff/biome/rubocop/sqlfluff/rust-clippy/dart-analyze/stylelint/eslint in tests/fixtures/autofix-smoke/<lang>/); ktlint blocked by #218. Workspaces are git-init'd so VCS-gated fixers (cargo fix) run.
