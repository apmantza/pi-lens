...
`tests/config/` tests on `windows-latest`. It remains advisory until the
`Run Windows Vitest subset` step outcome is `success` on master or pull
requests for seven consecutive runs.
...
Diagnostics have one model-facing surface, `lens_diagnostics`; `source` selects `session` or `lsp`, and `scope` selects `paths` or `workspace` (omitted scope defaults to the current turn's delta for `source=session`). `source=lsp scope=workspace` restricts to an explicit `path`/`paths` when given and sweeps the whole project only when neither is given — explicit paths always win (#2860 round 3, N2: the retired name's compat shim must never widen a caller's request into a sweep). `source=analyzers` was dropped (#2860 round 3, N3): it was observationally identical to `source=session` at every scope (the analyzer-fleet refresh is reachable only through the legacy `mode=full`+`refreshRunners` params, unchanged). The retired MCP name is intercepted through the canonical enabled-tool gate for one release, maps to `source=lsp scope=paths` (a no-op passthrough, never `scope=workspace`, so a no-`path`/`paths` call still errors exactly as master's `lsp_diagnostics` did), and records `lsp-diagnostics-compatibility` once per session. The retired name stays out of `TOOL_REGISTRY`, pi/MCP rosters, skills, and user-facing docs. (#2800 item 2)
...
- **Adversarial-review every PR before merge.** For every non-trivial PR, run a read-only review against the actual final head after rebases/merges and CI. The reviewer must challenge the invariants, cross-product matrix, security boundaries, failure atomicity, observability bounds, and composition with merged changes—not merely repeat the happy-path tests. Request changes for real P1/P2 findings; after repeated cross-product findings, return to the state-space model instead of applying isolated patches. Do not merge on green CI alone. **Beware the skipped-CI-on-conflict trap:** a `DIRTY` (merge-conflicted) PR cannot have its merge-ref built, so `ci.yml`'s `Lint & type-check` and `Unit tests` jobs are **silently skipped, not failed** — the PR shows only the always-runnable checks (CodeQL/Sonar) green, and a naive `gh pr checks | grep -cv pass` reads zero because a skipped required check is absent, not failing. Resolving the conflict and pushing re-triggers `ci.yml`; before merging, verify every gating check actually **ran and passed on the current head SHA** (`node scripts/ci-verdict.mjs <pr-number|sha>` — the ONE REST read this repo standardizes on, #2539; #2609/#2618 broadened it from just `Lint & type-check`/`Unit tests` to every non-advisory check-run), and never `--admin`-merge a formerly-DIRTY PR without that fresh green. **Conversely, SonarCloud is NOT a gating check** (confirm via `node scripts/ci-verdict.mjs`, whose table marks it advisory, or `gh api repos/.../branches/master/protection/required_status_checks` for the GitHub-required subset specifically); a red SonarCloud gate does NOT block merge and must not trigger correction rounds. Its `new_duplicated_lines_density` CPD over-flags inherently-repetitive code (lookup tables / policy maps — #1169's `FORMATTER_POLICY_BY_EXTENSION`), and its Automatic Analysis re-analyzes async so it **lags the head SHA** (a stale ERROR often clears once it catches up). Treat it as advisory: read the finding, don't contort correct code to satisfy CPD, and don't take a reviewer's assertion that "Sonar is a required gate" at face value — check `ci-verdict.mjs`'s own advisory classification (this cost two correction rounds on #1169). A `BLOCKED` merge-state means a required check (GitHub's own branch-protection sense — `Lint & type-check`/`Unit tests`) has not succeeded, most often because it never posted at all (#2575: BLOCKED with neither required check present on the head) — master protection is `strict: false` with no required reviews (probed 2026-09-06), so BLOCKED is never a BEHIND-base/branch-update condition here; that reads `UNSTABLE` instead (a non-required, i.e. non-gating-by-branch-protection, job still pending or red — #2618's own head read UNSTABLE mid-CI with the install matrix in flight). Never assume a non-required check explains a BLOCKED read (#2618: `Install test (*)` and `Production install build` gate via `ci-verdict.mjs` today, but neither is in branch protection's own required set, so their being red or pending produces UNSTABLE, not BLOCKED). **One review question the gates cannot ask (#1500):** when a call site hands a shared policy an outcome or classification it did not DERIVE from the evidence — `noteUnavailable("missing", "not-found")`, `available = false`, a hard-coded severity or verdict — ask what actually failed, and require either a derivation (`classifyProbeFailure`) or a comment justifying the assertion plus a record carrying the raw facts. Governed storage with a wrong classification looks identical to a correct verdict in the logs.
...
other agent runner. Include the contract text or an explicit repository-relative reference
in the task. A runner that keeps its own contract registry receives the same
two names in this order: `pi-lens-subagent`, then the selected role.
Repository instructions win if a runner-side copy has drifted.
...
worktree (2026-09-08: seven PRs landed this way through delegated codex workers). Follow "Prove filesystem isolation before coding subagents touch Git":
...
- *A direct push to master runs `npm run preflight` first.* Docs, config
  ...
  blocks every open PR's merge ref, and the oxfmt gate keeps formatting drift
  from reaching master.
...
- *Brief pre-flight by touched surface.* Read what the fix will touch and put
  ...
  separators from the host, `USERPROFILE`, UNC) because the Windows lane
  is advisory (#2536) and "green on Linux" is not "green on the
...
- *Follow-up rounds go to the same worker while it is alive* (a follow-up
  message, not a new delegation); a fresh worker on a resume loses
...
- *Contract edits land in the repo files* (`AGENTS.md`, `docs/pi-lens-*.md`);
  ...
  personal tool's flags, handles, issue numbers) never enter this file.
...
**Every outward-facing answer carries the AI-generated disclosure, not just PR reviews (2026-09-12).** The rule above was written for reviews and was read that narrowly for months, so agent-written issue triage, assessments, remainder comments, question-asking replies and PR comments went out with no disclosure at all. Anything an agent posts where a person outside this repo will read it says, once and plainly, that it was written by an AI agent with maintainer supervision. Put it at the end, in one line, so it does not crowd the substance; it is a disclosure, not a hedge, and it never replaces standing behind the content. A person deciding how much to trust a technical claim, or how much of their own time to spend answering a question, is entitled to know what produced it. This binds the orchestrator as much as a delegated reviewer: the comment that asks a reporter for a measurement is exactly the one where the disclosure matters most, because it is asking them to spend effort. Internal artifacts — the lane ledger, worker briefs, commit messages, our own issues nobody outside reads — do not need it.
...
**Clean up after merged PRs: the worktree AND the branch (local + remote).** Merged branches and their worktrees accumulate fast (a single burn-down session left 130+ worktrees). Note that `gh pr merge --delete-branch` SILENTLY fails to delete a branch a worktree still holds (`cannot delete branch … used by worktree`), so merging does not auto-clean when a worktree checks the branch out. Periodically and at session end: `git worktree remove` your own temp worktrees when done; `git worktree prune` dead entries; `git push origin --delete <branch>` for merged remote branches (works regardless of local worktrees); `git branch -d <merged-branch>` locally. For an external runner's worktrees, the runner auto-cleans unchanged ones — force-remove committed-branch ones only once their PR merged and the agent is no longer live.

**Shared-state design screen (runs before the catalog; 2026-09-06).** Before adding a second writer to any shared record — a cache entry, a latch, a session ledger, a mirror, a counter — ask once, at design time: does every actor need ONE canonical object, or can each actor own its own record and merge only at read? Shapes 12, 21, 22 and 24 below are four post-hoc entries for that single design failure (a second writer with no discriminator, a late loser overwriting, a write straddling a session boundary, a mirror refreshed outside the guard). Answer the question in the PR body's Blast radius before the first write lands; a reviewer who finds a second writer without that answer names the screen, not a shape.

### Recurring defect shapes — screen against these BEFORE you write code

- **Whole-module `vi.mock` factories** → `tests/config/vi-mock-export-sweep.test.ts` and `tests/support/vi-mock-export-gate.ts` (#2281, #2816): resolve relative `.js` specifiers to `.ts`, compare the production value-export surface with exports used by the test's direct production importers, and prefer an `importOriginal`/`importActual` spread so additions pass through. Skip `.mjs`, `.d.mts`, and `node:` specifiers. Keep admissions keyed by file, specifier, and sorted returned-property fingerprint; store the missing count, so production-export additions warn while factory-shape changes red. Regenerate with `npm run test:regen -- vi-mock`; #2272, #2782, and #2816 are the recurrences.
...
17. **A process-lifetime latch holding a session-scoped signal.** A module singleton can remember "once" after the session whose fact justified it has ended. *Screen:* every new once-latch answers "what resets it at `session_start`?". Put session dedupe in the degradation ledger, or reset it in the runtime-session reset block; do not hide it in an unregistered singleton (#1525/#1541/#1542). *e.g.* #2841: `cache-observability.ts`'s finding-identity set and cap latch are attribution substate, so `index.ts` resets them for the owned role after the concurrent-secondary gate; deleting that call leaves the repeated-finding count carried across a reused session id.
...
28. **Hot-path work hoisted for a cold-path record.** Data that only a failure or timeout record needs (which candidates were unrooted, what a wait skipped) computed up front on EVERY call so the record can name it. #2541 r1 resolved every LSP candidate's root on every touch (+3.7 ms per `.ts` file, unbounded on slow FS) to label a record that fires once per session. *Screen:* derive record fields inside the branch that emits the record; a hoist out of a cold branch onto the warm path needs a measured number in the PR body, and "already computed, just moved earlier" must be proven with an invocation count, not asserted.
29. **A cap whose counter is reset by the path it caps.** A retire/skip that `continue`s without marking the item seen, so the selector 30 lines below re-derives the same item fresh and the attempt counter restarts — the cap fires a ledger row and changes nothing (#2528 r2: a "too slow, retired" test target re-selected in the same turn via `related`, livelock intact under a green mutation test). *Screen:* every cap/retire records its decision where the selector reads (a seen set, a session-scoped retired set, a stamp on the persisted record) and the test seeds the input that lets the selector re-derive the item — a fixture that omits it cannot fail.
30. **Platform branching on a module-load-time constant, proven on one platform.** `const isWindows = process.platform === "win32"` at module scope cannot be forced per test the way a live check can; a test written on the dev box passes there and never exercises the other branch on the ubuntu lane (#2527 r2). *Screen:* platform reads are live (`process.platform` at call time) or the test forces the platform via `Object.defineProperty` + `vi.resetModules()` + a fresh dynamic import, once per case, and the PR body names the CI lane that runs each branch.
31. **A pull-only observability answer.** A change proven only by a query someone must run (`pilens_health` payload, a status command) leaves no trace in the streams a monitor or the log analyzer reads (#2513 → #2526: the config refactor ran a whole dogfood session invisibly). *Screen:* a new or replaced seam emits a success-path phase through `logLatency`/`logSessionStart` or the ledger; the PR body's Observability identifier must be a literal present in the diff (#2543).
32. **A path predicate with one case-folding branch and one exact branch.** `resolvedDir === resolvedHome` beside `path.relative(...)`: the ancestor branch folds drive-letter case on win32, the equality branch does not, so a lowercase-drive path (the VS Code URI form, present in real telemetry) walks past HOME (#2544 r1, every #250/#253 ceiling caller). *Screen:* one expression per predicate — `path.relative` covers equality and containment — and the regression test feeds both casings (the read-guard cross-form rule extends to every path comparison, not only map keys).
33. **A source-text assertion standing in for a runtime probe.** A test that slices the built or source file and asserts a shape ("exactly one `setTimeout` between this marker and EOF") is bypassed by hoisting the offending code above the marker, and is over-strict for everything below it (#2530 r2: two timers per call, guard green; the claimed "no runtime probe exists" was false — `vi.spyOn(globalThis, "setTimeout")` counts them). *Screen:* a structural assertion is admissible only when the PR body shows the runtime probe that is impossible; otherwise spy, count, or observe the effect. The reviewer's mutation must include hoisting.
34. **A guard that enumerates surface spellings.** A sweep whose detector lists the syntaxes it knows (`.keys().next().value`, `for (const k of m.keys())`) misses the next spelling every time — bare `for (const x of set)` walked through #2546 after #2442 had already added one spelling; the deadline-family allowance in #2530 accepted `withBudget(sweep(cwd, { signal }))` because the token `signal` appeared somewhere in the arguments. *Screen:* detect the SEMANTIC shape (a delete bound from the first element of an ordered container inside a size loop; a bound that receives BOTH a deadline and a signal on the same call), prove the guard red with the spelling the issue names AND one it does not, and when closing a symmetric hole (signal-only) check the mirror (deadline-only) in the same PR.
35. **A guard whose only red is on a platform CI never runs.** Unit tests run on ubuntu only (#2536). A regression test for win32 case-folding, drive letters or `.cmd` resolution that is a bit-for-bit no-op under posix semantics enforces nothing in CI: #2544 r2 shipped five such assertions plus a cross-drive guard that is a tautology on linux, so the shipped defect's only enforcement was a prose bullet. *Screen:* platform-dependent path logic takes an injectable `pathImpl` (default `path`) and the test table runs under BOTH `path.posix` and `path.win32` on every lane; the PR body names which lane reds each new assertion. Never substitute a source-text assertion (shape 33).
36. **A maintenance tool that rewrites a governance table on a count match.** A re-key / baseline-regenerate script that pairs old and new entries by per-identity COUNT equality launders a replacement: swap one flagged site for a brand-new one in the same symbol, the count is unchanged, the old exemption (and its reason) is zipped onto the new occurrence, and the guard goes green (#2530 r3: `rekey-hook-await-exemptions.mjs`, header claiming one-to-one refusal). *Screen:* a rewrite is accepted only when the content-keyed identity (`rel#symbol:hash(ownLine)`) is unchanged and only the neighbourhood suffix moved; every other delta is printed with its source line and refused; the tool ships with a fixture test that proves the laundering case is refused. The same rule governs `depcruise --output-type baseline` regeneration: diff per edge, never by count.
37. **A literal control byte written into tracked source.** A fixture or separator authored as a raw byte below `0x20` (NUL, ESC, BEL, …) inside a `.ts`/`.mjs`/`.md` file makes the file look binary to some tools, hides matches from others, and reads as mojibake in review — #2528 r4 and #2552 r4 both wrote a literal `U+0000` into a template literal the same day, independently, because neither fixer thought of it as a class. *Screen:* author every control character as a source escape (`\0`, `\x1b`, `\t`, `\n`, `\r`) or via `Buffer.from([...])`, never as the raw byte between the quotes; a byte-level population sweep over the whole tracked tree is the only mechanism that catches this — Markdown and some `.ts` fixtures are not reliably reachable by ast-grep/oxlint rules, which classify by parsed token, not raw byte. *Detect:* `tests/config/tracked-control-bytes.test.ts` scans every Git-tracked `*.ts`/`*.mjs`/`*.md` file byte-for-byte and fails on any byte below `0x20` other than tab, LF, or CR (#2571).
38. **A guard, ratchet or baseline that a one-line data edit or a convenient fixture satisfies away.** The guard is real, its red proof is real, and admission to it costs nothing: a line in a baseline JSON, a renamed identifier, or a fixture that never crosses the boundary the guard exists to defend. #2584 r1 wrote every control-byte fixture as `0x00`, so the `< 0x20` boundary itself was never tested; #2585 r1 shipped 28 `makeTouchFileMock = vi.fn` call sites whose only purpose was to rename a literal out of its own sweep's reach; #2585 r2 shipped a ratchet where a brand-new file with a hand-rolled double plus ONE added baseline-JSON line passed the whole suite with no reason recorded; #2583 r2 shipped mutation-vacuous branches under checked "mutation-proof" boxes. In every case the checklist was honest about the guard and silent about its door. *Screen:* before shipping a guard, run the CHEAPEST evasion against it yourself — edit the data file it reads, launder the identifier it matches, pick the fixture that avoids the boundary — and quote that attempt. Admission must never be a data edit alone: require a SECOND, machine-checked part in a different file (a reason string naming an issue plus a header line in the admitted file), and pick fixtures that straddle the boundary rather than sitting well inside it. *Detect:* `tests/config/lsp-service-double-sweep.test.ts` (the two-part admission gate, with one test per (state × writer) cell) and `tests/clients/flake-shape-ratchet.test.ts` (the precedent it reuses) (#2585, #2584). *Also:* a floor or ratchet calibrated against a population that a ROUTINE process deletes — the 4.1.4 changelog roll consumed 151 `.changelog/*.md` fragments and the tracked-control-bytes Markdown floor of 80, calibrated while they existed, went red only on the bump PR (2026-09-07). Calibrate against the population that survives the process, and say which files are excluded and why.
39. **A walk-up result used as an eligibility gate.** A `findNearest*`/walk-up answers "which X OWNS this directory"; using the mere existence of that result to answer "does this directory GET X's settings" hands every descendant the owner's configuration. #2580 shipped this twice in consecutive rounds. Round 2 gated uv's `UV_PROJECT_ENVIRONMENT` and workspace `.venv` on "a `pyproject.toml` exists at or ABOVE root", so `<mono>/frontend` — a real LSP root, since `PythonServer`'s `NearestRoot` detector returns it off a `requirements.txt` — inherited `<mono>`'s environment ahead of its OWN `.venv` and an activated `VIRTUAL_ENV`, which also made uv's documented process-wide `UV_PROJECT_ENVIRONMENT` export capture every subdirectory on the box sitting under some Python project. The same round's `isUvWorkspaceMember` answered "member" on `projectRoot === workspace.root` BEFORE consulting the walk at all, so every non-project subdirectory of a workspace root inherited the workspace `.venv`, `exclude` included — an identity comparison short-circuiting a walk-derived predicate is the same defect wearing an `===`. *Screen:* a walker that can answer an eligibility question returns the DISTINCTION, not just the location (`{ projectRoot, isStartDir }`), and the gate names the narrow half; ask of every walk-up consumer whether the caller's root being BELOW the found root changes the correct answer, and if it does, that is a second question needing a second field. Both halves are only visible as a grid, which is why this shape pairs with the state-space step: enumerate root-position x each ambient input, give every cell an expected answer BEFORE writing the gate, and drive the cells — the defect lives in the cells no single-case test visits (#2583's grid is 72 cells; 15 of them were red). *Detect:* grep for a walk-up result consumed as `x !== undefined` (or `x?.field === true` on a location-only field) where a later line uses `x.someRoot` as though it were the caller's own root; and for an `=== someRoot` early return inside a predicate whose other branches read the walk. *e.g.* #2580 rounds 2 and 3 (#2583).
40. **A tool root resolved at a different seam than its child spawn.** Runners, formatters, and LSP servers must call `resolveToolCwd(kind, tool, file, ctx)` so marker discovery, the dispatch-root fallback, the real-git fallback, and the `$HOME` ceiling cannot drift between consumers. The seam preserves tool-owned per-file configuration discovery, logs `cwd <kind> <tool> cwd=<path> reason=<...>` once per resolution key per session, and records one bounded `tool-cwd-resolution` degradation per fallback tool and session. *Screen:* sweep every runner and formatter child spawn, php-cs-fixer config carriage, and LSP root detector for a direct `ctx.cwd`, `path.dirname`, or private walk; mutation of one consumer, the log throttle, or the fallback ledger must make its corresponding test red. The existing AST sweep remains the population guard, and the seam state-space table covers nested markers, root-only markers, foreign files, symlinks, per-file versus per-cwd discovery, and LSP servers with or without `rootMarkers` (#2777, extending #2691).

    *Also:* the on-write TEST runner was a member of this population that no sweep covered — `clients/test-runner-client.ts` never imported the seam, and the #2777 AST sweep's needle only checks that SOME `cwd` is passed, not where it came from, so a spawn handed the dispatch root reads as conforming. When a whole-project runner's cwd cannot follow its manifest, express that as the runner's own launcher markers rather than a post-hoc override of the seam's answer: an override leaves the seam's log line naming a directory the child never ran in. *e.g.* #2871 (2026-09-10).
...
42. **A rule keyed on one language.** A seam, cache admission, test matrix or tool contract written for `.ts`/tsserver when the behaviour belongs to every LSP-backed language in `clients/language-registry.ts`. #2823 r1 keyed the #2817 dependency re-sync on TypeScript; rust-analyzer, pyright and gopls hold a stale module graph after a git checkout exactly the same way. *Screen:* write the rule against the registry ("has import facts", "every live server holding the document"), name which entries have the facts and which fall to the honest fallback, and add one non-TypeScript row to the matrix; a language-specific branch is allowed only with the registry field that justifies it named in the code. *Detect:* grep new code for `\.tsx?\b`, `"typescript"`, `tsserver` outside `clients/lsp/config.ts`'s server entries and the TypeScript runner; a test file whose only fixtures are `.ts` for a rule that names "LSP" or "server". *e.g.* #2823 r1 (2026-09-09).
    ...
    *Also, outside LSP:* the same shape keyed a whole subsystem's selection on one language's spelling. `RUNNERS` had no per-runner statement of which file kinds it can run, so the table's declaration order answered for every language in a polyglot repo, and the java root markers listed only the Groovy `build.gradle` while `kotlin` listed both spellings — a Kotlin-DSL Gradle module therefore anchored at the workspace root. *Screen:* a marker/extension list for one language family carries EVERY spelling that family uses (`build.gradle` + `build.gradle.kts`, `settings.gradle` + `settings.gradle.kts`), and a table of tools keyed by name declares the kinds each row claims rather than leaving the answer to iteration order. *e.g.* #2870 (2026-09-10).
...
**`clients/cargo-manifest.ts` is the one Cargo.toml reader (#2466, folded #2473, closed #2498).** `extractTomlTableSection`/`parseTomlStringArray` (table-section slicing, string-array parsing) moved out of `clients/lsp/server.ts`'s rust-analyzer workspace-root hoisting (#1671/#1693), which now re-imports them instead of keeping a private copy — do not add a SECOND regex TOML reader; this one already covers table-section slicing, string-array/scalar parsing, and workspace-inheritance detection. `clients/review-graph/workspace-modules.ts`'s `scanCargoModules`/`detectWorkspaceType` (monorepo module-graph construction) used to carry an independent regex reader (`extractTomlArray`/`extractTomlSection`/`extractTomlString`) — a third copy, folded onto `readCargoWorkspaceMembers`/`readCargoDependencyNames`/`readCargoPackageName` (#2473). That fold also fixed a latent defect: the old `extractTomlString` was NOT table-scoped, so a member manifest whose `name` key appeared under an earlier non-`[package]` table (a `[[bin]] name = "..."` or `[package.metadata.*]` block preceding `[package]`) silently returned the wrong crate name for the module graph; `readCargoPackageName` is table-scoped like every other reader here. Review round 2 (#2473, PR #2480) hardened the fold: `extractTomlTableSection`/`parseTomlStringArray` now normalize CRLF→LF and strip `#`-to-EOL comments (quote-aware) before any regex runs — the pre-fold per-line reader did both and the fold's single multi-line-regex rewrite silently dropped the comment-strip pass, so a commented-out array entry (`# "member",`) stayed live; both the heading and its terminator are now anchored with `[ \t]*` rather than bare `^` so an indented (but valid) TOML heading is read, not silently treated as absent — and, since ECMAScript's multiline `$`/`^` already treat a bare `\r` as a line terminator, that SAME anchor change is also what made a CRLF manifest read correctly (review round 3 correction: the CRLF→LF normalize itself is defensive belt-and-braces, not a fix for a real match failure — verified by mutation, removing it leaves every test green). `readCargoWorkspaceMembers` gained `[workspace]` table-scoping and a sibling `readCargoWorkspaceExclude`, both now reused by `clients/lsp/server.ts`'s `cargoWorkspaceDeclaresMember` instead of that file hand-composing the same `extractTomlTableSection`+`parseTomlStringArray` pair itself — the same single-source-of-truth violation #2473 closed for the other two readers. `RustWorkspaceRoot`'s walk-up still carried a THIRD instance of the pattern four lines above `cargoWorkspaceDeclaresMember` — a bare `/^\s*\[workspace\]/m.test(parentCargoContent)` presence check that #2473's fold missed — closed by #2498's `hasCargoWorkspaceTable(content)` export, now the one presence check every caller — that walk-up, both of `resolveCargoPackageEdition`'s own checks, and `workspace-modules.ts`'s `detectWorkspaceType` — shares; `tests/clients/cargo-manifest.test.ts` sweeps `clients/` for a reintroduced `\[workspace\]` regex literal outside this file. `workspace-modules.ts`'s `detectWorkspaceType` also stopped using a bare `content.includes("[workspace]")` (true for a commented-out heading, wrongly short-circuiting past a real npm/pnpm workspace sitting next to it); it called `extractTomlTableSection(content, "workspace") !== undefined` directly for two rounds (#2473 review round 3 fix below) until round 2 of #2498/#2520's tidy PR routed it through `hasCargoWorkspaceTable` too, closing the last caller that reimplemented the shared reader's own presence idiom instead of naming it. Review round 3 fixed a second sentinel-ambiguity defect in that same presence check: `extractTomlTableSection` used to return `""` for BOTH "table absent" and "table present but empty" (e.g. a bare `[workspace]` heading as the last line of the file with no trailing newline), so `!== ""` misread the second as the first — `detectWorkspaceType` and both `resolveCargoPackageEdition` "does this manifest declare `[workspace]`" checks now compare `!== undefined` (now via `hasCargoWorkspaceTable`) instead, and `extractTomlTableSection`'s return type is `string | undefined`; `readCargoDependencyNames` also dropped a redundant, non-quote-aware `line.split("#", 1)[0]` re-strip left over from before the fold (its input is already comment-stripped by `normalizeToml`). `resolveCargoPackageEdition` walks up from a file via the shared `findNearestMarkerRoot`/`isAtOrAboveHomeDir` primitives (home-ceiling guarded, never a private walker) to the nearest `Cargo.toml`; reads `[package] edition` directly, or — when it declares `edition.workspace = true` — checks whether that SAME manifest is also the workspace root (`[package]`+`[workspace]`+`[workspace.package]` in one file, a common non-virtual-workspace-root shape) before climbing ancestors for the nearest one that DECLARES `[workspace]` (Cargo's own rule — not merely the nearest ancestor Cargo.toml, which may be an unrelated intermediate package) and reading ITS `[workspace.package] edition`. Every resolved value is checked against rustfmt's actual `--edition` enum (2015/2018/2021/2024, `SUPPORTED_RUSTFMT_EDITIONS`) — not a bare four-digit pattern, which would let a typo'd or too-new edition (e.g. `"2019"`) pass through and turn every `.rs` format into a hard failure; `rustfmtFormatter.resolveCommand` carries the validated value through `--edition` because bare `rustfmt <file>` (unlike `cargo fmt`) does not read the manifest's edition itself and silently rejects newer-edition syntax under its 2015 default. Manifest-value-into-argv sweep (#2466): ruff/black already self-discover `pyproject.toml`'s `target-version` from the file path they are given (no pi-lens translation needed); gofmt has no version-dependent flag at all; clang-format's own default is `-style=file` (ancestor `.clang-format` search) with no flag required; prettier/biome/ruff/shfmt already carry detected indentation through `indentationArgs` (#1144); psscriptanalyzer-format already carries `-Settings` (#1572 F2); ktlint self-discovers `.editorconfig` the same way (no carriage needed — its Spotless `build.gradle` detection is a tool-SELECTION axis, ktlint vs ktfmt, not an argv-carriage one). Two gaps found, tracked separately (not this fold's scope — see each issue for current status): **ktfmt** — its CLI did not read the `googleStyle()`/`kotlinLangStyle()` selection out of a project's `build.gradle{.kts}` ktfmt-gradle-plugin block (`hasKtfmtConfig`'s `KTFMT_GRADLE_FILES` check only detects ktfmt's PRESENCE, never which style it selects) and `ktfmtFormatter.resolveCommand` passed no style flag — fixed by #2468, see `clients/gradle-ktfmt-style.ts` below. **php-cs-fixer** — `detect()` walks up for `.php-cs-fixer(.dist).php` via `findNearestContaining`, but `resolveCommand` never passes `--config <path>` and `formatFile` spawns with `cwd` = the file's own directory, which php-cs-fixer does not walk up from on its own — fixed by #2472, see `clients/php-cs-fixer-config.ts` below. Round 2 of #2580 generalized the table-PRESENCE check one level: `hasTomlTable(content, tableName)` is the shared `!== undefined` half of `extractTomlTableSection` (`tableName` is a regex SOURCE, so a dotted table escapes its dots — `"tool\\.uv\\.workspace"`); `hasCargoWorkspaceTable` is now a one-line caller of it, and `clients/python-environment.ts`'s uv-workspace discovery calls it for `[tool.uv.workspace]` instead of carrying a private `hasUvWorkspaceTable` that was the same body with a different table name. **The workspace-MEMBER glob matchers are folded onto ONE dialect-parameterized helper (#2591).** `matchesWorkspaceMemberPattern(pattern, relativePath, dialect)` in `clients/path-utils.ts` — next to `nameMatchesMarkerGlob`, the sanctioned marker-glob seam — is now the only "does this workspace declare this directory as a member" matcher; `clients/lsp/server.ts`'s `cargoWorkspaceDeclaresMember` and `clients/python-environment.ts`'s `isUvWorkspaceMember` are thin callers passing `CARGO_WORKSPACE_MEMBER_DIALECT` / `UV_WORKSPACE_MEMBERS_DIALECT` + `UV_WORKSPACE_EXCLUDE_DIALECT`, and neither `segmentGlobToRegExpSource` nor a private minimatch options block survives. The dialect is an OBJECT, not a flag, because the tools diverge on four independent axes: `**` (cargo: a pattern containing one never matches, #1671 F6, preserved byte-for-byte; uv: crosses components, a trailing `/**` still requiring at least one), whether `*`/`?` may cross `/` (uv `exclude` only — rust `glob`'s `require_literal_separator: false`), whether `[abc]` is a character class (uv) or a literal bracket run (cargo, whose pre-fold compiler escaped it), and pattern normalization (cargo strips trailing slashes; uv runs `normalize_path`). Component-count equality is an ENTAILMENT of "wildcards confined to one component", not a fifth axis — it is not a separate flag, and the table vectors that pin it red when `[^/]*` is widened. Case-sensitivity and dot-permissive matching are hard-coded rather than parameterized: every dialect agrees on them, and a field with one value across every constant is configuration that can never be wrong. uv's two option sets are pinned to `astral-sh/uv@3c979abda4530fe9bf3d92e9bcf5c5575e3b3126`, `crates/uv-workspace/src/workspace.rs` (`is_included_in_workspace` vs `WorkspaceExclusions::matches`); the separator-crossing `exclude` `*` that pi-lens used to document as an unimplementable minimatch limitation is now implemented, the one intentional answer change in the fold. The fold also drops minimatch-only extensions the uv path had inherited by accident — brace expansion, extglobs, leading-`!` negation, leading-`#` comments — which uv's own `glob` crate does not support either, so that moves uv toward upstream; **The matcher is NON-BACKTRACKING and there is no compiled whole-path regex left in it (#2603).** A pattern compiles to a list of BOUNDED steps (a literal, a `?`, a `*`, a one-character class regex, or the epsilon fork a `**` emits) and `matchesWorkspaceMemberSteps` decides it with one memoized (step, position) table, O(steps x characters), filled by a single backward double loop — no recursion, no re-entry, no nested quantifier for an engine to explore. The anchored regex it replaced gave the same answers (15840-cell old-vs-new differential across all three dialects, 0 diffs) but emitted a nullable `.+` per `**` and explored 2^N splits of a non-matching path: #2591 round 2 collapsed CONSECUTIVE `**`s the way upstream does (`rust-lang/glob@cfa2a58f2e44373573f657ec25b3621e44714dee`, `src/lib.rs:672-684`), but a collapse cannot fire across a separating component, so an INTERLEAVED chain still cost 124900ms for `("**/*" x12)/zzz` against a 40-component path (35096ms at x10 on the #2603 box, and uv `exclude` did not finish x4 in five minutes) through `detectPythonEnvironment`, which is awaited with no timeout. The collapse was DELETED with the regex: it was a speed normalization, the step table is linear with or without it, and removing it reds nothing — adjacent `**`s denote exactly what one does. Do not add a second matcher for a subset of the dialects; the one evaluator serves all three, and the character-class axis is the only place a `RegExp` is still constructed (one class, no quantifier, tested against a one-character string). A pattern whose class cannot compile answers `false` for every path (fail closed, declaring no member and excluding nothing), because a glob character class is not a JS one: `[z-a]` is a legal glob with an empty range and an illegal RegExp. `tests/clients/path-utils.test.ts` pins the whole thing with a per-dialect vector table plus a 2156-cell differential against the pre-fold `minimatch(…, { dot: true })` call whose every remaining disagreement is enumerated with a reason; `tests/clients/workspace-glob-nonbacktracking-budget.test.ts` holds the wall-clock half in both uv dialects, admitted to the flake-shape ratchet's `elapsed-time-assertion` detector because the defect it pins is time-only. **`clients/review-graph/workspace-modules.ts`'s `expandWorkspacePattern` deliberately stays separate** and is NOT a fourth dialect: npm/pnpm `workspaces` entries are EXPANDED against the filesystem (one `readdir` of the directory preceding the first `*`, filtered by the project ignore matcher and capped at `MAX_WORKSPACE_CANDIDATES`, yielding the directories that exist and hold a manifest) rather than tested against a candidate path — it answers "which directories does this pattern name", not "does this pattern name this directory". Folding it in would mean handing a pure matcher a filesystem, which is a bigger seam change than the duplication it would remove.
...
**Structural-provenance rule invariant (#2576):** A tree-sitter security rule may ADD suppressions on same-file AST provenance, but never trade an existing one for it — narrowing a shipped exemption regresses every user who relies on it (#2577 review). The Python SQL rule therefore keeps both name-shaped exemptions (`session.execute(...)` by receiver name; a `select/insert/update/delete(...)` builder in argument position) and layers provenance on top: when an unshadowed `sqlalchemy.orm.Session` / `sqlalchemy.ext.asyncio.AsyncSession` import and a typed receiver prove the relationship, `query` and `execute` are suppressed — but only after reading the ARGUMENT, never on the receiver alone. `query` takes mapped classes, so it does not require a builder, but it is suppressed only when its argument cannot be SQL text: a string, a concatenation, a `%` or `.format` composition, an f-string, a non-literal `text(...)`, or any name the enclosing function binds all stay diagnostic. `execute` is suppressed only for a builder call or a name bound exactly once to one (`stmt = select(User)`). An attribute callee (`sa.select(...)`) counts as a builder only when its object resolves to a proven `sqlalchemy` import — otherwise any object's `.update(...)` launders SQL — and no builder call may wrap composed SQL. The sink set stays `execute`/`executemany`/`query`/`raw`: `scalar`/`scalars` take a statement object and adding them only manufactured findings on `client.scalar(stmt)`. psycopg composition is suppressed only for an unshadowed `psycopg`/`psycopg2` `sql` import, a static `sql.SQL` template, and `sql.Identifier`-only format arguments. Missing, duplicate, shadowed, or reassigned provenance remains diagnostic.
...
by its `(advisory)` NAME SUFFIX (`PR body (advisory)`, `Vale prose lint
(advisory)`, `OSV scan (advisory)`), not
by a vendor allowlist; one head's rollup really does carry DUPLICATE check
names, so `resolveLatestByName` (`scripts/lib/ci-checks.mjs`) picks the newest
by `startedAt` and fails closed on an unorderable disagreement, because
`new Map(list.map(...))` is last-wins on array order and called an in-flight
re-run green; and `direction=desc` is
...
- **18 tools in a source checkout (17 in an installed package):** `pilens_analyze`
  ...
  / `pilens_ast_grep_replace`, `pilens_lsp_navigation`,
  ...
  **dual-surface** — also registered as pi agent tools (`tools/module-report.ts`,
  ...
  The per-file resolution root is CONFINED to `cwd`, not assumed to nest under
  it (#2520): a `file` outside `cwd` has no containment relationship to the
  workspace the view is `cwd`-labelled as, so resolving from the file's own
  directory regardless answered from a foreign tree's config while `documents`
  omitted the workspace's own. `effectiveConfig` now checks containment via
  `isSameOrWithin` (`clients/lsp/server.ts` — the same comparator the LSP
  session-root registry gates enrollment with) and returns
  `file: { error: "file is outside cwd" }` instead of a resolved view for a
  `file` that fails it; `EffectiveConfigView.file` is
  `EffectiveFileView | EffectiveFileViewError`, narrowed via
  `isEffectiveFileViewError`. A `file` nested INSIDE `cwd` is unaffected — the
  project walk still passes back through the workspace's own document on its
  way up, which is the SUPERSET property the module doc now states as
...
- **Warm-build staleness guard (#535).** The warm server lives for weeks, so it
  ...
  `lsp_navigation`; the CacheManager/latency log — the rest),
...
- **Same-workspace warm attach (#822, opt-in soak).** `PI_LENS_WARM_ATTACH=1`
  ...
  `lens_diagnostics mode=full` sweeps use the same seam
...
- **An external report is assessed against the code before it is accepted OR dispatched, and the load-bearing unknowns are ASKED (2026-09-12, #3000).** Read the report, then verify its claims and your own diagnosis against the source; say in the reply which statements you checked and what you found, so the reporter can correct a wrong reading early. Then name the one or two facts that would CHANGE the design, and ask them explicitly — marked optional, with the reason they matter, so the reporter can judge whether to spend the effort.
- **Do not dispatch work that depends on an unanswered load-bearing question.** Asking is not enough on its own. On #3000 the question ("is the pinned tool version statically readable?") was asked and a fixer was dispatched before the answer arrived; the answer showed the project's `libs.versions.toml` entry is the ktlint-GRADLE PLUGIN version, not a CLI version, so the briefed remedy would have compared `14.2.0` against `1.8.0` and reported a large, entirely false skew. **A confidently wrong claim is worse than the silence the fix exists to remove.** Either wait for the answer, or scope the dispatch to the part that does not depend on it and say in the brief which part is deferred.
- **When the answer arrives, mirror it on the issue and point the worker at the issue, not at your paraphrase.** A worker cannot verify a mid-task instruction but it can verify an issue comment; a scope change delivered any other way is declined by design. Tell it explicitly that the comment outranks the original brief, and ask it to report "I built X, the correction means Y, here is what I changed" rather than quietly reshaping its work.
- **Treat a reporter's reasoning as authority to cite, not raw material to paraphrase.** On 2026-09-12 three external reports each out-reasoned the lane they fed: #2968 supplied Windows 24H2 measurements nobody here could produce and retracted its own weaker evidence unprompted; #2982's contributor found a root cause smaller than either remedy the review proposed; #3000 supplied the framing the fix now uses — "establish agreement, otherwise decline" rather than "read the declared version", which turns an awkward ecosystem from a permanent exception into the ordinary case of a property not being provable. Quote such reasoning into the brief with its source so the fixer can weigh it, and credit it in the PR body.
...
npm run logs:smells   # scripts/analyze-pi-lens-logs.mjs — scans ~/.pi-lens/*.log for operational smells (diagnostic-blockers, slow-hook-path ≥5s, slow-runners ≥2.5s, cascade-slow-graphs ≥1s, lsp-availability-noise, read-guard-friction, config-resolution). Flags: --since 3d (default 2d), --limit, --json. READ CRITICALLY: much volume is user-project diagnostics (not pi-lens bugs) + self-noise from temp:pi-lens-smoke-* (my own --install cold-tool runs). Real pi-lens smells: cascade-slow-graphs + slow-hook-path (cold LSP + cascade hot-path).
...
**And never SPELL a project-data path in text you hand an agent or a user (#2521).** The same rule applies to display, not just to writes: the turn-end actionable-warnings and code-quality advisories carried a hardcoded `Details written to .pi-lens/cache/<name>.json`, which is correct only in `getProjectDataDir`'s legacy arm — so in a project with no `.pi-lens/` directory (the common case) agents ran `cat .pi-lens/cache/actionable-warnings.json`, got `No such file or directory`, and never read the report that was written successfully one line earlier. `displayProjectDataPath(cwd, ...segments)` (`clients/file-utils.ts`) is the one seam: it composes through `getProjectDataDir(cwd)` so the shown path cannot disagree with the written one, renders `/`-separated relative-to-cwd when the store is inside the project (the legacy arm's short spelling) and otherwise absolute with `homeRelativePath`'s `~` fold, so the default `~/.pi-lens/projects/<slug>/…` store never leaks the account name into agent context (#2440 F5). Prefer naming the TOOL over the file wherever one exists — `lens_diagnostics mode=delta` reads both of those caches, so an advisory that leads with it means the agent needs no layout knowledge at all, and the resolved path is a stated fallback rather than the instruction. Guarded by `tests/clients/data-dir-display-path-sweep.test.ts`, a sweep-kit `auditRegistry` walk of `clients/`+`tools/`+`mcp/` that fails any `.pi-lens/` string literal not covered by a reasoned file-level exemption (today: the two config-location modules, the CLI-flag help text naming `~/.pi-lens/config.json`, and one path-matching predicate).
...
- **`config_resolved` is the config stack's POSITIVE record (#2526).** The Phase 0 config stack (#2429/#2440/#2456/#2513) used to prove itself only by the ABSENCE of `PILENS_CFG_*` / `config-ignored` rows — correct for a canonical-only layout, and indistinguishable from "resolution never ran". `loadLSPConfig` — the ONE funnel every production resolution goes through (`initLSPConfig` at session start and per served root, the warm MCP boot's `ensureLspConfig`, `ensureLSPConfigInitialized` on the first edit) — writes one `config_resolved` phase per session AND SERVED ROOT carrying `documents[]` (`{tier, file, legacy}`), `countsByTier`, `recordCount`, `deniedServers` and `resolveMs`, plus a matching `config resolved …` line in `sessionstart.log`. **Redacted structurally, never by mode:** the payload comes from `summarizeConfigResolution` (`clients/config-resolve.ts`), the same projection `pilens_effective_config` embeds for its `documents`/`provenanceCounts` — one definition, so the on-demand answer and the session record cannot drift. It is a projection of a resolution the caller ALREADY performed (#2513's facade rule: no second `resolvePiLensConfig`), and both writes are synchronous, so the session-start hook path gains nothing to await.
  **The claim, and where it re-arms (round 2, F1/F3).** `claimPhaseOncePerSession(phase, scope)` lives in `latency-logger.ts`, keyed on `(phase, normalizeFilePath(root))` — `scope` is REQUIRED (round 3, S3): a phase-only claim recorded the first served root and silently dropped every other one, so a legacy config in a second MCP-served project could never surface its smell. The re-arm (`resetOncePerSessionPhases()`) is called from **index.ts's `session_start` closure**, on the line after `resetCurrentPhaseForSession()`, behind the #473 concurrent-secondary gate — **not** from inside `handleSessionStart`. index.ts resolves the session's config in `ensureLSPConfigInitialized` BEFORE it calls the handler; a re-arm inside the handler used to fire between a session's own two resolutions (the ensure's row, then the deferred `loadLSPConfig`'s second row for the same session). The warm MCP server (`clients/mcp/session.ts` calls `handleSessionStart` directly, never through index.ts's closure) therefore keeps ONE session identity — and one shot at each served root's claim — for the life of the process; a root claimed once stays claimed until the process exits. `ensureLSPConfigInitialized`'s OWN memo (`index.ts`'s `_lspConfigInitializedCwds`, a `BoundedSet` capped at 128 distinct cwds) is a separate, unrelated per-process short-circuit for the host-session path: once more than 128 distinct roots have been served in one process, the oldest entries evict, and a session for an evicted root is treated as "first" again — genuinely re-resolving (and re-claiming, since a NEW session's re-arm always clears the claim anyway) rather than merely appearing to. This is a real, accepted, boundedness gap, not a correctness bug: a process serving that many distinct roots is not the common case, and the memo's job is caching the FIRST resolve cheaply, not policing every later one.
  **The session join is real evidence, not a prediction (round 3, S1).** Round 2 had `handleSessionStart` publish a `session_start config-resolution session=<id> expected=<bool> reason=<...>` line, with `expected` PREDICTED from the same `no-lsp`/subagent/warm-attach flags the resolution paths themselves gate on. The mirror drifted: quick and minimal mode's SECOND session in a process schedule no resolution at all (the `_lspConfigInitializedCwds` memo above skips the synchronous resolve past the first session, and only FULL mode's deferred `setImmediate` reschedules one), so the prediction kept saying `expected=true` for a session that was never going to resolve, and `npm run logs:smells` flagged a real, healthy session every time under `PI_LENS_STARTUP_MODE=quick` or `=minimal`. `loadLSPConfig` now publishes its own `session_start config_resolution_pending session=<id> root=<normalizeFilePath(cwd)>` line, as the FIRST thing it does — before anything it calls can throw — so the mark fires exactly when a resolution is genuinely attempted, never merely likely: a session that never reaches `loadLSPConfig` at all (that second quick/minimal-mode session) never gets a mark, and is silently excluded from the smell rather than counted against. The mark and the `config_resolved` row are stamped from the SAME `currentSessionRecordId()` read a few lines apart in one call, so they can never disagree about which session they belong to.
  **The join key is (session, root), not session alone (round 4, #2552 review MEDIUM).** A warm MCP process keeps ONE session id for its entire life but calls `loadLSPConfig` once per SERVED ROOT (`ensureReady` per root, `mcp/server.ts`) — round 2's F3 already made the row's CLAIM per-root for exactly this reason, but round 3's pending mark and the analyzer's join were still session-id-only, so one root's `config_resolved` row silently cleared every OTHER root's deficit under the same session id: two pending marks for one MCP process, one row, and the smell stayed silent. The pending mark, the row's sessionstart line, and the row's `filePath` all now carry the SAME `normalizeFilePath(cwd)` value (`clients/lsp/config.ts`'s `recordConfigResolved` computes it once and reuses it for the claim scope, the row, and the line, so the three can never disagree); `npm run logs:smells`'s `config-resolution` smell joins `pendingSessions`/`resolvedSessions` on `configJoinKey(sessionId, root)` — a `${sessionId} ${root}` composite key (`scripts/analyze-pi-lens-logs.mjs`) — rather than on session id alone. A pending or resolved line missing `root=`/`filePath` is silently excluded from its side of the join rather than guessed into a wrong root.
...
**Turn-end test selection: built-in integration/e2e exclusion, and runner-error vs real-failure delivery (#2522).** The bounds above cap HOW MUCH the test batch may fire — they say nothing about WHAT. `getTestRunTarget`'s three strategies (failed-first/related/self) resolve to any test file the naming convention finds, so a candidate whose own test happens to be an integration/e2e suite that spawns an external process gets fired exactly like a unit test (a delegated dogfooding turn resolved `tests/integration/opencode-delegate.test.ts`, which spawns `opencode` and needs a configured provider — 17 s to fail on a box without one, reported as a blocking failure on an unrelated model-switch turn). `TURN_END_EXCLUDED_TEST_GLOBS`/`isExcludedTestTarget` (`clients/test-runner-client.ts`) is ONE hard-coded built-in list — `**/integration/**`, `**/e2e/**`, `**/*.integration.*`, `**/*.e2e.*` — with NO per-project config knob (maintainer decision); the turn-end candidate loop (`runtime-turn.ts`) checks every RESOLVED target against it before the `existsSync`/cap checks, regardless of which strategy produced it, and logs `turn_end: … → test target excluded (integration/e2e), skipping spawn` naming each one. The match is case-INSENSITIVE (`tests/Integration/`, `tests/E2E/` are the same hazard, and the same files on the case-insensitive filesystems Windows and macOS ship by default) and fails CLOSED for any target that resolves outside the project root. The batch-wide wall budget (`TEST_RUNNER_BATCH_BUDGET_MS`, same file) is reconciled to 20 s — well under the turn and an order of magnitude below the per-target 60 s spawn timeout — rather than adding a second budget alongside it.

**Turn-end test batch: the deferral is a mechanism, not a comment (#2522 review round 2).** Lowering the batch budget is only safe because the batch now HAS somewhere to put what it cut. `runTestTargetsBounded` (`runtime-turn.ts`) used to return a skipped COUNT and throw the unrun targets' identity away, so a single target slower than the budget produced no result, no cache record and no output — every turn, forever — and a PARTIAL batch (some targets settled clean, the rest cut) fell through to the clean branch and was published as "all tests passed", clearing the `--lens-guard` test-failure blocker on a run that never finished. Three invariants now hold, and each has its own test: (1) the batch owns an `AbortController` threaded into every target's spawn (`TestRunRequest.signal` → `safeSpawnAsync`), so reaching a bound KILLS what is in flight rather than leaving it to burn its own 60 s timeout; the outcome is latched closed at that instant so a killed run can never push into a results array the caller has already consumed. (2) The killed plus never-dispatched set is returned BY IDENTITY as `deferred`, persisted on the `test-runner-findings` record as `deferredTargets`, and dispatched FIRST on the next turn — ahead of failed-first/related/self, under the same `TEST_RUNNER_MAX_TARGETS` cap — and cleared as they are dispatched, so nothing is carried forever and nothing is silently dropped. **`retireTestFindings` (`runtime-context.ts`) is the ONE statement of what survives a delivery** — `testRunGeneration`, `deferredTargets`, `retiredTargets` — and BOTH `consumeTestFindings` (the in-process context hook) and `acknowledgeTestFindings` (the MCP Stop-hook commit, `mcp/session.ts:451`) call it. Do not re-inline it: round 2 taught only the consume copy and left the acknowledge copy writing `{ content: "", testRunGeneration }`, so on the Stop-hook route — including the ack-only branch where `handleTurnEnd` never runs that turn — the deferral set was wiped before anything could dispatch it. A superseded batch does not drop its cut set either: `supersededByNewerGeneration` MERGES it into the live record (`mergeDeferredTargets`, higher `attempts` wins) before returning, because the newer batch's own pre-run write already reset `deferredTargets` to `[]` and never saw the older batch's targets. (3) The clean branch requires an EMPTY deferral set: a partial batch records `N test target(s) … deferred to the next turn` with advisory framing and does not touch the commit gate. A cut batch is a recurring event at a 20 s budget, so it uses `incrementDegradationCount`, not `recordDegradationOnce`. And the deferral is capped, because otherwise it is a LIVELOCK: a target whose own runtime exceeds the whole budget — this repo's `tests/index-integration.test.ts` is one, at 26.4 s against a 20 s budget, resolved by `related` whenever `clients/runtime-turn.ts` is edited — would be deferred, put first, cut, and deferred again forever, burning the full budget in spawned runners every turn. `DeferredTestTarget.attempts` counts the cut batches and `TEST_RUNNER_MAX_DEFERRALS` retires the target from turn-end selection with a counted degradation naming it. **A retirement that does not outlive the turn is not a retirement (round 3, F1).** Round 2's retire branch logged, counted, and `continue`d — leaving no record anywhere — so the candidate loop THREE LINES BELOW re-resolved the very same file through `related`/`self` in the SAME turn and ran it with a fresh (undefined) attempt count: steady state was a 3-turn cycle (cut, cut, "retire", run, …) that spawned and killed the slow suite on every turn, never reached the clean branch, and delivered the deferral advisory every turn. The retired set is therefore PERSISTED as `TestRunnerFindingsCache.retiredTargets` and consulted by the candidate loop (`retiredKeys`, keyed through `normalizeMapKey`) so no strategy can re-select a retired target. Both `deferredTargets` and `retiredTargets` carry a `sessionId` stamp, and round 4 makes the rule about that stamp exact: **a foreign entry is IGNORED, never deleted.** Two entry points fire `handleTurnEnd` against the same project with different identities — `index.ts` passes pi's stable `ctx.sessionId`, `clients/mcp/session.ts` passes none and so stamps `runtime.telemetrySessionId` — so "another session" is an ordinary LIVE state, not a leftover from yesterday. Round 3 filtered both lists to the current session and wrote the FILTERED lists straight back, so one turn taken through the other route erased everything the first route had recorded (`retired: []`, `deferred: []`) and the retired suite was immediately re-selected through `related`: the livelock, restored by a write. Filtering therefore happens at SELECTION only; `writeTestFindings` re-reads the LIVE record and merges into it, and an entry leaves the list only when it is this session's, this turn settled it (dispatched, retired, or unrunnable), and this turn's own write is not re-asserting it. Because nothing prunes foreign rows any more, both lists are bounded by `TEST_RUNNER_MAX_PERSISTED_TARGETS`, applied at that one writer with this session's rows ordered LAST so the bound sheds foreign rows first. Note also that pi's session id SURVIVES a resume: a sequential replacement of the session does NOT re-arm a retirement on the `index.ts` route, and only a genuinely new session id does — do not describe the stamp as "expires when the session ends". Every write merges for the same reason: a batch that publishes CLEAN used to write `deferredTargets: []`, which flattened the cut set a concurrent older batch had just handed over — the `supersededByNewerGeneration` merge only fixed the hand-over, not the newer batch's own publish that erased it a moment later. And a carried target the per-turn cap has no room for is HELD, with its attempt count untouched: round 3 folded that case in with "missing file, excluded, unknown runner" and dropped it, so a carry larger than `TEST_RUNNER_MAX_TARGETS` lost its tail every turn under a `dbg` line saying the files no longer resolved. Finally, "the batch closed" is not "the target was cut" (round 3 F4, re-founded in round 4): the answer is stamped by the `.then` attached to `run`'s promise AT DISPATCH — that promise's first continuation, which only a microtask can precede, while every path to `finish()` arrives on a macrotask or later. Deciding it downstream and repairing the misclassification with a `setImmediate` hop is what produced the next bug: `safeSpawnAsync` resolves SYNCHRONOUSLY with `"Spawn aborted before start"` when its signal is already aborted, and `runTestFileAsync` awaits `resolveExec` before the spawn, so the hop folded a target that never ran back in as a completed one. If you add a new deferral-style carry-forward anywhere, it needs all of this: the same attempt cap (retrying cannot make work fit a budget it structurally exceeds), a retirement that survives the turn, a session stamp that filters without deleting, and a bound on the persisted list. Separately, `TestRunnerFindingsCache.runnerErrorOnly` distinguishes a batch made ENTIRELY of runner errors (timeout, missing provider/binary, a rejected promise — `TestResult.error` with `failed === 0`) from one containing a genuine failing test (`failed > 0`): `peekTestFindings`/`consumeTestFindings` (`runtime-context.ts`) read it to deliver the former as advisory ("could not complete last turn") instead of the blocking "Test failures detected last turn — fix before continuing" framing, since the agent introduced nothing to fix. A batch with even one real failure keeps the blocking framing regardless of any runner errors alongside it. **#2532 extended the SAME per-result seam to the other two surfaces that render one `TestResult` independently of the batch-level `runnerErrorOnly` flag:** `testResultToProjectDiagnostics` (`clients/project-diagnostics/runner-adapters/runner-findings.ts`, feeding `lens_diagnostics mode=full`) rendered a runner-error result as `semantic: "blocking"`, and the `--lens-guard` `mergeGitGuardTestFailure` call in `handleTurnEnd` ran unconditionally whenever `failures.length > 0`, so an all-runner-error batch with NO prior blocker still flipped `hasBlockers: true` — the identical event the turn-end message already delivered as advisory read as "COMMIT BLOCKED" under `--lens-guard`. `isRunnerErrorResult` (`test-runner-client.ts`: `result.failed === 0 && !!result.error`) is now the one predicate both surfaces read — `testResultToProjectDiagnostics` per `TestResult` (a mixed batch can hold both a runner error and a genuine failure; the batch-level `runnerErrorOnly` flag cannot distinguish them), and the `--lens-guard` call site gated on `!runnerErrorOnly` (equivalent at batch granularity, since `runnerErrorOnly` is `!hasRealFailure` and `hasRealFailure` is `failed > 0` for any result in the batch). A genuine failing test — `failed > 0`, even with no per-test `failures[]` detail — still classifies blocking on every surface. **Round 1 review (S1) found a THIRD and FOURTH surface the identifier-only class sweep above missed:** `TestRunnerClient.formatResult` (`test-runner-client.ts`) and the turn_end dbg summary (`runtime-turn.ts`) each re-derived the same fact locally as `error && passed === 0 && failed === 0` — a spelling that ALSO required `passed === 0`, so a runner error reported alongside partial passes (real pytest exit 2 "Interrupted" after some tests already passed) fell through to the normal pass/fail rendering and silently dropped the interruption. Both now route through `isRunnerErrorResult` too; a partial run reads `⚠ Could not complete tests: <error> (N passed before)` rather than a clean pass. **The lesson: grepping for an IDENTIFIER (`isRunnerErrorResult`, `mergeGitGuardTestFailure`) cannot find a site that re-derives the same VALUE with different variable names — a class sweep for a classification defect must grep the boolean EXPRESSION SHAPE** (`error &&`, `passed === 0 && failed === 0`), not just the names of the functions already known to matter. Also corrected: `isRunnerErrorResult`'s doc previously claimed `failed` is 0 "by construction" whenever `error` is set — false (`parsePytestOutput` sets `error` from the exit code independently of the parsed counts, so a counted failure can carry `error` too); the real rule is exactly `failed === 0 && !!error`, and a counted failure stays blocking regardless of `error`.

**Which runner a file gets is decided by its KIND and its own module root, never by the dispatch root's config files (#2870).** `detectRunner` used to probe the project root's config files alone, so a repo carrying both a `go.mod` and a Gradle build handed every file to whichever runner `RUNNERS` declares first: a `.java` test under `src/test/java/` ran as `go test -run . ./app/gw/src/test/java/com/x/pkg`, answered `FAIL <pkg> [setup failed]`, and was delivered as a blocking failure on every Java edit. Two facts are resolved independently and in this order. (1) The GATE is the file kind: every `RUNNERS` entry declares `kinds: FileKind[]` in `file-kinds.ts`'s vocabulary, and a runner may only be handed a file whose kind it claims — which is also what stops a `README.md` or a `fixture.yaml` under a test directory from becoming its own test target, since `detectFileRole` is extension-agnostic and calls any file under `/test/`, `/tests/`, `/spec/` a test. The gate is NOT the resolved root (defect shape 39): a `.java` file in a go-only repo resolves to the workspace root, where `go.mod` lives, and must still get no target. (2) The ANCHOR is `resolveLanguageRootForFile` — the same per-kind, cached, workspace-clamped walk the dispatch runners resolve their roots with, never a private walker — and the availability memo is keyed on that resolved root, so one module's positive verdict cannot answer for another's. A file OUTSIDE the dispatch root keeps the pre-#2870 resolution (every runner, probed at the dispatch root) so #2522's exclusion layer still gets a target to fail closed on. Related: go's own infrastructure verdicts `FAIL <pkg> [build failed]`/`[setup failed]` are runner errors — advisory, never a counted failure — and no longer depend on go having printed the substring `error` for that classification; the exit-code-distrust guard in `parseGenericRunnerOutput` is skipped for them, as it already is for every other runner error. **The child's cwd is a SECOND value, resolved through `resolveToolCwd("runner", …)` with the runner's own `configFiles` as the markers (#2871).** `runTestFileAsync` used to hand `safeSpawnAsync` the dispatch root, so `RUNNERS.go.args` built `./tools/tapctl/internal/x` against a root module that does not own that package; the seam resolves the module instead, and only `args()` and the spawn read it. The failed-target ledger (`recordResult` → `getTestRunTarget`) and binary/interpreter resolution keep the DISPATCH root — re-keying the ledger per module hides a recorded failure from the turn that selects targets, and a hoisted `node_modules/.bin` lives at the repo root, not the package. A resolution outside the dispatch root is clamped back to it (the seam's `$HOME`/file-dir fallback is right for a formatter on a foreign file and wrong for a test runner). A whole-project runner declares `spawnCwdMarkers` — its LAUNCHER (`gradlew`, `mvnw`) rather than its manifest — because moving its cwd changes which project is built and `./gradlew` is resolved against the child's cwd: a module with a `build.gradle.kts` but no wrapper would be `spawn ./gradlew ENOENT`.
...
**Cached-blocker freshness gate (#1631).** A cached inline blocker is a verdict about its file *and everything that file imports*, yet every prior invalidation path keyed on the file alone — so when the cause was fixed in a dependency (especially out-of-band, where the dependency is never dispatched) the stale verdict re-served for the rest of the session (#1561's dependency-axis remainder, live instance). Before a cached blocking finding is re-served at turn end, `clients/blocker-freshness.ts` (`sweepInlineBlockerFreshness`) stats the file and its forward imports (resolved via the parse layer — no reverse-dependency index, so #1561's tests-free-index blocker does not apply) against the verdict's `recordedAtMs`; a drifted entry is DEMOTED, not dropped (#1419 demote-not-drop): `stale = true`, served out of the authoritative blocker channel as a `[stale — re-run to confirm]` advisory, and the gate never re-pulls the LSP itself (re-querying an LSP whose in-memory dependency document is itself stale would regenerate the stale verdict). The SAME gate lives at the widget-store cache layer (`reconcileStaleWidgetDependencyBlockers`, paired with `reconcileStaleWidgetFiles` at the `mode=all` read site) so `lens_diagnostics mode=all` cannot serve a blocking entry a same-minute `mode=full` sweep contradicts. One bounded `blocker_freshness_sweep` latency phase (plus `blocker_freshness_widget_gate` for `mode=all`) names the revalidated/kept/truncatedImports counts (`retired` was removed — the gate is architecturally demote-not-drop, so it never deletes an entry and the count would always read 0). `isBlocking` answers false for a stale finding, which is the single predicate both the footer tally and the mode=all blocking count consult. The full dependency-axis INVALIDATION (actively re-verifying consumers when a dependency dispatches clean) remains deferred to #1561. **Population gap (#1790).** The turn-end sweep's population was built solely from `RuntimeCoordinator`'s inline-blocker map — a live-dispatch-only store — so a blocking row a workspace-diagnostics CACHE HIT wrote straight into `widget-state.ts`'s `files` map (never touching the inline-blocker map) rendered in the widget while the sweep logged `total:1 kept:1` and never saw it. `sweepInlineBlockerFreshness` now accepts `options.additionalEntries`, populated by `runtime-turn.ts` from `widget-state.ts`'s `getWidgetBlockingFilesForSweep`, deduped by file path (`normalizeEphemeralMapKey` — cheap and consistent with the widget store's own key, not `normalizeMapKey`'s realpath/walk-up) against the inline-blocker map so a file present in both is drift-checked once. Review round 1 (#1790 F1) found the dedup DROPPING the widget row on a duplicate path: a live dispatch writes BOTH stores for one verdict, and `markInlineBlockerStale` only ever touches `RuntimeCoordinator`'s map, so the widget's own `isBlocking` for the file stayed true even after the inline entry demoted — the same ghost, one store over. A duplicated path now CHAINS the widget demote onto the inline entry's demote instead of discarding it: one drift check, both stores written. Review round 2 (#1790 F5) found the chain itself still ghost-prone: it fired even when the inline entry it chained onto was INELIGIBLE for the drift check (already latched `stale`, unstamped, or carrying no recorded provenance) — the sweep's own gates short-circuit before ever calling `demote()` on those, so the chained widget demote silently never fired, either as a permanent forever-ghost (the stale latch never re-arms) or reappearing the instant an unrelated inline blocker shared the file. A duplicate now only chains when the inline entry can actually answer the widget row's question (`canSubsumeLspWidgetRow`); a duplicate that cannot gives the widget row its own separate population entry, so a file can legitimately count twice when the two stores disagree. **Self-drift axis (#1561 remainder).** #1631 review F4 excluded a non-`"lsp"` record from the sweep entirely, reasoning that an ast-grep secret match does not stop being true because a file it IMPORTS changed. True of the import axis, but it discarded the self axis with it: a tree-sitter verdict about a file's own syntax is invalidated by that file's own bytes changing, and nothing else could clear such a record — `retireInlineBlockerOnConfirmedClean` needs `coveredSources` to be a superset of the record's sources, `coveredSourcesForCheck` builds that set from the LSP server registry, and no registered server has id `"tree-sitter"`, so the retire could never fire however many clean checks ran (live: a `ts-incomplete-assertion` blocker re-served every turn for a whole session against a file proven clean by grep, LSP, and a passing test). The sweep's third gate is therefore "has recorded provenance", not "is all-`lsp`", and the SOURCES pick the axis: all-`lsp` gets own-file plus forward imports, anything else (tree-sitter, ast-grep, mixed, `"unknown"`) gets own-file only; a record with no recorded sources stays fail-closed and untouched. The self axis is CONTENT-CONFIRMED and RE-ARMING, and carries its own `"self-drift"` reason. Content-confirmed: mtime moving is not evidence a byte moved (a `touch`, a checkout restoring identical bytes, a no-op formatter pass all move it), so `detectSelfDrift` applies #2449 round 2 F7's rule — confirm against content before acting — at its cheap `size` tier; a `recordedSize` stamped by `recordInlineBlockers` is the baseline, an absent one reads `unverifiable` and changes no state, and a same-size change reads unchanged and keeps the record authoritative, which is the pre-#2982 behaviour. Re-arming: `setInlineBlockerSelfDriftStale` re-derives every turn and un-demotes a record whose bytes come back, as `setInlineBlockerPastEofStale` does for a transient shrink-then-restore, which is what keeps this axis OUT of the #1950 delivery cap. That cap retires permanently after `DEPENDENCY_DRIFT_MAX_DELIVERIES` and suits recoverable LSP dependency drift; a self-drift record can carry ast-grep or tree-sitter security provenance, and retiring one because an advisory was shown three times would walk a hardcoded secret out of turn-end rendering with no dispatch to re-raise it (#2982 review). A record that heals needs no bounded-noise retirement. Defect shape 24 is satisfied by per-writer semantics plus the sibling-gate discipline (never touch another gate's demotion, never heal one you did not make), not by reusing a reason string. The commit gate is unaffected: `updateGitGuardStatus` counts `getInlineBlockersSnapshot().length` with no `stale` filter, so a demoted record still gates a commit, and a test pins that because it is what makes widening to security provenance safe. Sweep counts gain `selfHealed` and `selfUnverifiable`. This is also why the chain predicate split from the loop's: a widget row is pure-LSP by construction and needs the import walk, so a self-only inline entry is drift-checked on its own axis yet may not stand in for that row. The self axis confirms at two tiers (defect shape 6): `size` from the stat already taken, then a sha256 of the bytes when size matches, because a same-LENGTH edit is the common shape and a size-only tier would leave a genuinely changed record authoritative. The baseline is captured OFF the synchronous dispatch path — `recordInlineBlockers` reads nothing and returns its `recordedAtMs` stamp; `runtime-tool-result.ts` does the bounded read and calls `setInlineBlockerContentBaseline`, which drops a late baseline whose stamp no longer matches. Every expired bound, missing baseline, or over-budget file yields `unverifiable`, which changes no state and is counted as `selfUnverifiable`. The sweep's filesystem work runs under `bounded()` with `TurnEndDeps.signal` threaded through `BlockerFreshnessOptions.signal`, and a per-sweep `SELF_DRIFT_HASH_BUDGET_BYTES` bounds the aggregate read (defect shape 9). The widget-origin write path is `markWidgetFileBlockersStale`, not `reconcileStaleWidgetDependencyBlockers` (that one stays mode=all-only) — same store, same demotion, different trigger, so this reuses the write rather than re-implementing it; a comment there pins the single-file-baseline assumption it depends on for a future writer. The two stores are injected rather than cross-imported because `widget-state.ts` already imports FROM `blocker-freshness.ts` (`collectForwardImportMtimes`); importing back would cycle.
...
- **There is exactly ONE project-local tool-bin walker, and it stops at HOME by default; config-file walkers keep reading past it (#2514/#2517/#2544).** `findLocalBinUpwards` (`package-manager.ts`, plus `findLocalBinsUpwards` — the same generator drained for every match instead of stopping at the first) is THE walker: it takes `{ windowsExt, homeDir, binDirs, isWindows, ceiling }`, and `binDirs` is the ONLY thing that differs per ecosystem — `NODE_MODULES_BIN_DIRS` (default; also reused by stylua/taplo/knip/jscpd, the ast-grep sweep's PROJECT walk in `runner-helpers.ts` and, via `findNodeToolBinary`, madge), `VENDOR_BIN_DIRS` (Composer, `windowsExt: ".bat"` — `formatters.ts`'s `findInVendorBin` and `runner-helpers.ts`'s `resolveVendorToolCommand`), `VENV_BIN_DIRS` (Python, `windowsExt: ".exe"` — `formatters.ts`'s `findInVenv`; the fixed-lookup consumers are below, not this list). **Those three constants are also the single source of truth for the directory NAMES** — the venv list alone had three independent spellings, two of them behind a module-load `process.platform` branch (defect shape 30), so `VENV_BIN_DIRS` lists both the POSIX and Windows layouts unconditionally and nothing re-lists them. Every WALKING consumer is a three-line delegation that passes those constants and nothing else. Do NOT hand-roll a seventh copy: the SIX copies that existed before #2544 review F1 had already drifted (only `findLocalBinUpwards` did `path.resolve(startDir)`, so a relative cwd silently resolved nothing in the others; #2514's HOME ceiling had to be re-applied by hand to each; and `vp` was resolved with OPPOSITE ceiling policies depending on whether `formatters.ts` or `oxlint.ts` asked). Candidate order is EXT-OUTER, DIR-INNER, and `isWindows` is injectable precisely so the ubuntu lane can assert that order — with a live `process.platform` read there is only ONE candidate name off Windows, so the order is unobservable and a swapped-loops mutation stays green (#2544 F4). Two shapes in this family are deliberately NOT walks and must not be converted without a reported defect: `runner-helpers.ts`'s `resolveNodeToolCommand` and `biome-client.ts`'s `getBiomeBinary`, both of which join onto the caller-supplied project/sub-package root. The ceiling is DEFAULT-ON `isAtOrAboveHomeDir` (`homeDir` injectable, default `os.homedir()`) — a bin match found at or above `$HOME` can never be the current project's own installed dependency, so escaping the project here means STOP. This is the opposite default from `findLocalToolConfig` (#2472 review round 3, F1), which is UNCEILINGED by default because a config file living directly at `$HOME` (`~/typos.toml`, `~/sgconfig.yml`, …) is the user's legitimate global config that the underlying tool would read on its own — config-file lookups keep reading past HOME, tool-resolution (bin) lookups stop. `#2517` tracks building the single per-walker escape-policy table (stop vs keep-reading) this bullet and `findLocalToolConfig`'s doc comment currently each state independently.
  - **`ceiling: false` is for exactly ONE caller, never a project resolver (#2544 round 4 F1).** `runner-helpers.ts`'s `buildSgLocalBins` walks TWICE: once from `process.cwd()` (ceilinged, as above) and once from `_thisDir` — pi-lens's OWN install directory — with `ceiling: false`. `pi install npm:pi-lens` installs into the HOME-level manifest (`~/package.json` → `~/node_modules/pi-lens`), so pi-lens's bundled `@ast-grep/cli` shim can sit AT `~/node_modules/.bin/ast-grep` — exactly the directory the ceiling exists to stop the PROJECT walk at. The ceiling is a project-tree policy ("a bin found at or above HOME can never be THIS PROJECT's own dependency"); it says nothing about pi-lens's own install location, so applying it to the own-install walk silently returned nothing for the HOME-level layout (round 2 shipped this regression: PATH, `findGlobalBinary` and npx do not cover this location either). `_setThisDirForTests` (test-only) overrides `_thisDir` so this is provable without writing into the test process's REAL, shared `node_modules`.
  - **`findLocalBinsAt` / `findLocalBinAt` are the non-walking siblings — single directory, no ceiling, because a lookup that never climbs can never escape the project (#2544 round 4 F2).** `createVenvFinder` (`runner-helpers.ts`) and vulture's `venvCandidates` (`dead-code-client.ts`) are FIXED lookups at `cwd`/`root` — never ancestor walks. #2544 review round 3 folded them onto the walking `findLocalBinsUpwards`, reasoning a monorepo package should find its repo-root venv the way project-root discovery would; that was an unrequested behaviour change with NO #2514 defect behind it (#2514 is about the HOME ceiling, not about climbing at all), and the inversion is real: `~/code/app` with no venv of its own would resolve `~/code/.venv/bin/ruff` — an unrelated SIBLING project's interpreter — ahead of PATH. Reverted in round 4 F2. `binDirs`/`windowsExt`/`VENV_BIN_DIRS` stay shared with the walking helpers so the directory shapes have exactly one spelling; only the walk itself differs.
- **`isAtOrAboveHomeDir` is ONE `path.relative` expression — never add a `===` fast path in front of it (#2544 F1).** `path.relative` already answers "is home inside `dir`, or the same directory", and it answers with the PLATFORM's equality semantics: case-folding on win32, case-sensitive on POSIX. A `path.resolve(dir) === path.resolve(homeDir)` short-circuit is case-SENSITIVE everywhere, so on Windows the lowercase-drive form VS Code URIs produce (`c:\Users\jane`, the form real `latency.log` records carry) missed the equality test, `path.relative` folded it to `""`, and the accompanying `rel !== ""` clause then rejected it — the ceiling answered "not at home" FOR HOME ITSELF, and every #250/#253 caller (`findNearestMarkerRoot`, `findLocalToolConfig`, `workspace-topology`, `startup-scan`, `project-lens-config`, and #2514's bin walkers) climbed straight past `$HOME`. Same shape as `isUnderDir` in the same file; keep the two in step, and note that `rel.startsWith("..")` (rather than `rel === ".." || rel.startsWith(".." + sep)`) is the house form used by both. The helper takes an optional third argument `pathImpl` (default `path`) for TESTS ONLY: every assertion that can catch this defect — drive-letter folding, the cross-drive `isAbsolute` clause — is win32-only, and the authoritative Unit tests lane is ubuntu, so through the ambient `path` module CI's diff for the fix was structurally ZERO. `tests/clients/path-utils.test.ts` runs one 14-row table under BOTH `path.win32` and `path.posix` on every lane; do not replace that with a `process.platform` skip or a source-shape assertion (shapes 30/33). The missing Windows Unit tests lane itself is #2536.
...
**A lockfile regenerated by a different npm major than CI's silently omits
optional platform bindings (2026-09-09, #2789 → b9bef8df).** npm 9 on the
maintainer box wrote a vitest 5 lock without `@rolldown/binding-openharmony-arm64`;
the runner's npm added it during the clean-clone `--omit=dev` install and the
"changed tracked source files" guard fired — on master, after merge, because
the PR head had passed. Regenerate locks with the CI-pinned npm
(`npx -y npm@<pin> install --package-lock-only`), never with whatever `npm`
is on PATH, and never touch the primary checkout's `node_modules` while
delegated worktrees share it. The completeness gate lives in preflight and
`check:lockfile` (issue filed the same day).
...
- **GOTCHA — pi resolves each `pi.skills` entry relative to the PACKAGE ROOT (the dir holding `package.json`), not the extension entry file and not `dist/`.** pi's resolver is `PackageManager#collectFilesFromManifestEntries` in `@earendil-works/pi-coding-agent` `dist/core/package-manager.js`: a non-glob entry is `resolve(root, entry)`, where `root` is the `packageRoot` passed down from `addManifestEntries(entries, packageRoot, …)` (the non-glob branch is identical in 0.78.1, 0.84.1 and 0.85.1; the glob branch was refactored into `expandPackageGlob` in 0.85.1, adding dot-segment filtering and a sort — so only the non-glob resolution is version-stable; entries containing `*`/`?` take the glob branch instead, and a directory entry is walked for `SKILL.md`). So the correct value is **`["./skills"]`** → `<packageRoot>/skills`. **Anything with `../` escapes the published package**: `"../../skills"` from `~/.pi/agent/npm/node_modules/pi-lens` resolves to `~/.pi/agent/npm/skills` (missing), and from a `git:` clone at `~/.pi/agent/git/github.com/apmantza/pi-lens` to `~/.pi/agent/git/github.com/skills` (missing). The user-visible consequence is NOT "no skills": the extension's own `resources_discover` handler (#205, `resolvePackagePath(import.meta.url, "skills")`) registers `<packageRoot>/skills` regardless of the manifest, so on standard layouts all four skills load anyway (verified on real pi 0.85.0/0.85.1, 2026-09-06). The real defect was that when the escaped path landed on an EXISTING directory (the user's project `skills/`, `<cache>/npm/skills`) pi adopted that foreign tree as a pi-lens package resource and, `loadSkills` being first-wins by name, a same-named entry there shadowed the real skill. Skills register iff `skills/` sits beside the `package.json` nearest the loaded entry file; when it does not (entry copied out of the package by a managed cache), BOTH paths yield zero, silently — tracked as its own defect (silent zero-skills degradation, filed 2026-09-06). **How #199 got this wrong, and how it survived four releases (#2587):** #199 reasoned from the *guess* that resolution was entry-file-relative (`resolve("dist/index.js", entry)`, so "climb two levels") and then wrote a `tests/packaging.test.ts` guard that replicated **that guess** rather than pi's code, so the test asserted the bug and stayed green. The lesson is the standing one for external contracts: read the upstream resolver, don't paraphrase it, and never let the test double encode the same assumption as the fix. The guard now replicates `resolve(packageRoot, entry)` and additionally asserts the entry cannot escape a synthetic install root; `scripts/install-selftest.mjs` re-runs the same resolution **from the installed package** in install-smoke.
- Guarded by `tests/packaging.test.ts` + the CI install-test (tarball ships `dist/index.js` + root `skills/`, no `.ts`, compiled entry loads "from dist") + `scripts/install-selftest.mjs` (skills resolve inside the *installed* package, across npm/pnpm/bun/yarn).
...
- **Python project environments apply without shell activation (#717, closes #2407, refs #1513, #2580)** — `clients/python-environment.ts` resolves a project's interpreter without invoking a package manager, in this fixed candidate order: `UV_PROJECT_ENVIRONMENT`, an explicit uv workspace `.venv`, `VIRTUAL_ENV`, `CONDA_PREFIX`, then `<root>/.venv` and `<root>/venv`; a candidate whose interpreter file is missing falls through to the next. **The two uv candidates exist only when the passed root IS a uv project root** — its NEAREST `pyproject.toml` is the directory itself (`findUvWorkspace`'s `isStartDir`). A `pyproject.toml` merely sitting somewhere ABOVE the root makes that directory a subdirectory of a project, not a project, and **nothing below a project root inherits anything from it**: not the project's `UV_PROJECT_ENVIRONMENT`, not an ancestor `.venv`, not a workspace `.venv`. Gating on "a pyproject at or above the root" instead is the exact defect rounds 2 and 3 of #2580 each shipped once: uv's documented CI/Docker recipe exports `UV_PROJECT_ENVIRONMENT` process-wide, and `PythonServer`'s `NearestRoot` detector hands this resolver `<mono>/frontend` off a `requirements.txt` while the `pyproject.toml` sits at `<mono>`, so the loose gate lets one image-level variable capture every subdirectory on the box that happens to sit under some Python project. A relative `UV_PROJECT_ENVIRONMENT` is anchored at the member's workspace root, else at the project root — which, given the gate, is the passed root itself; never the cwd. An explicit workspace's `.venv` is inherited ONLY by its declared, non-excluded members. `isUvWorkspaceMember` must NOT short-circuit on `projectRoot === workspace.root`: the workspace root is a member of its own workspace only in its capacity as a PROJECT, it already gets uv's settings through `isStartDir`, and its own `<ws>/.venv` is reached by the ordinary project candidate at the same path — the shortcut instead promoted every non-project subdirectory of the workspace root to a member, `exclude` included (#2580 round 3, S2). The walk stops at the injected `homeDir` via `isAtOrAboveHomeDir` (#625), so an unrelated `[tool.uv.workspace]` in `/tmp` or `$HOME` cannot claim the projects beneath it. Membership globs follow uv's dialect, pinned to `crates/uv-workspace/src/workspace.rs` at uv `3c979abda4530fe9bf3d92e9bcf5c5575e3b3126`: patterns are normalized (a leading `./` is not part of the pattern) and matched case-SENSITIVELY with `*`/`?` confined to one path component but matching a leading dot — minimatch's defaults plus `dot: true`, so no `nocase` flag, which would diverge from uv on Windows rather than match it. The full state space (root position x `UV_PROJECT_ENVIRONMENT` x `VIRTUAL_ENV` x own-`.venv`, 72 cells) is enumerated and driven cell-by-cell in `tests/clients/test-runner-python-environment.test.ts`; extend that grid rather than adding a one-off case. It supplies a child-only `PATH` and `VIRTUAL_ENV` to Python LSP, standalone Pyright, and pytest processes; never mutate `process.env`, because different roots can select different environments. `PythonServer.spawn()` prefers project-environment pyright/basedpyright/ty binaries before broader candidates. `TestRunnerClient` runs pytest through the detected project interpreter itself and uses generic `python -m pytest` only when no project environment exists. PEP 723 script environments remain outside this resolver because uv stores them in a content-keyed cache without a stable path marker. ty stays opt-in: it is not in the installer registry, and its `resolveAndLaunch` call keeps `allowInstall:false`, so managed pyright remains the default when the project and PATH provide no Python LSP. No `initializationOptions` are sent to ty because it has no stable `pythonPath` equivalent (astral-sh/ty#2032); its child environment and cwd drive interpreter discovery.
...
11. **A test named for a hope, not a behavior.** `it("should do X")` reads as a wish about code that is the real source of truth; `it("does X if Y")` is a line of the specification the code must satisfy. *Screen:* every test name is a declarative, present-tense statement of behavior and its condition ("does X", "does X if Y", "keeps X when Y", "rejects X"), never `should`. The suite is the blueprint: if every name reads as a true sentence about the system, a rewrite that passes the suite is the same system, and an assumption the suite does not state is an assumption the rewrite will break (added 2026-09-06; the ~50 legacy `should` names are tracked for renaming, not a governance sweep — no shipped defect names this recurrence yet).

12. **A guard test that asserts the outcome and not the reason.** `expect(decision.block).toBe(false)` is green whether the guard allowed for the reason under test or failed open on an unrelated branch; a byte-identical real-home log is green whether the hermetic sink caught the write or the lifecycle stopped writing. *Screen:* a test that pins a guard, gate or sink asserts the DISCRIMINATOR — the `reasonCategory` on the allow record, the row that landed in the scratch sink, the phase that fired — and is red under the failure mode it exists to catch, not only under the guard's deletion. *e.g.* #2644 (allow asserted, `resolveGitToplevel → undefined` stayed green until the reason was pinned), #2641 (real-home-unchanged asserted, `prepare` with the writer removed stayed green until the scratch-sink row was pinned) — both tests-only PRs, both one prescribed round, 2026-09-06. *Detect:* a boolean/emptiness assertion on a guard's output with no assertion on which branch produced it; a "nothing changed" assertion with no "the right thing happened elsewhere" twin.
...
- **A docs list that mirrors a registry asserts MEMBERSHIP against that registry, never a count** (#2919): `tests/docs/features-counts.test.ts` diffs the `docs/features.md` formatter and LSP lists and the `docs/mcp.md` tool table by name against `ALL_FORMATTERS`, `LSP_SERVERS` (non-auxiliary, by `role`), and `TOOL_REGISTRY`, with hand maps only as staleness-checked spelling overrides (an override whose key no longer names a registry member reds; a redundant identity override does not).
