...
were removed from the live contract during the context reduction. The compact
live rules remain in AGENTS.md. This snapshot is retained for historical
rationale and is not an active instruction source.
...
- *Plegma reads are token-budgeted* (#417). Do not call `plegma result` merely
  to check lane occupancy. Use `watch --next --mine` for settlement
  notifications and the bounded status or summary surface when available; use
  `result` only when the transcript or handoff artifact is required. Never use
  an unbounded `list` as a status substitute.
...
## Archived fixer contract (2026-09-14)

The compact role contract replaced the prose-heavy version below. The
archived text is historical and is not an active instruction source.
...
## Archived reviewer contract (2026-09-14)
...
## Archived investigator contract (2026-09-14)
