...
| Chaining a check and a commit or push with `;` or a pipe | Three times on 2026-09-25: a commit with `lint=1`, a push with the flake-shape ratchet red, and a push reported done by `echo $?` of a later `grep`. Gate with `&&`, or read the check in its own call; the guard is #3471 |
...
| Adding real spawns or waits to a test without running the flake-shape ratchet | #3464's `git init` fixture raised a spawn count and pushed red, because pre-push does not select the ratchet (#3472). Run `tests/clients/flake-shape-ratchet.test.ts` whenever a test gains a spawn, timer or never-settling promise |
